...
A group input can read an implicit field, a context value or a named attribute
when nothing is connected to it — what Blender's interface editor calls the
*Default Input* and *Default Attribute*. The `tree.inputs.*` factories take
these as `default_input=` and `default_attribute=`, accepting either Blender's
identifier strings or the members of `nodebpy.Default`:
...
The constructor mirrors the interface: `Default.POSITION` and
`Default.attribute("UVMap")` as parameter defaults document the fallback, and
passing them through leaves the group node's socket untouched so the fallback
applies. `Default.attribute(...)` also works for `tree.outputs.*`, where it
names the attribute an output is stored to.
