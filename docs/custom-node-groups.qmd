...
modifier.node_group = scattered_spheres(count=128)
```

Unlike `CustomGeometryGroup`, nothing is cached -- every call creates a fresh node group data-block (Blender will suffix repeated names with `.001`, `.002`, ...).
