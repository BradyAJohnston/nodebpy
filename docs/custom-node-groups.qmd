...
Subclassing `CustomGeometryGroup` is worthwhile when you want a reusable *node* with a typed constructor. Sometimes you just want a plain node group -- parameterised by Python values at build time -- that you'll assign to a modifier or only re-use within the current script. The `geometry_tree`, `shader_tree` and `compositor_tree` decorators turn a function into a factory for such a group.

The decorated function receives the `TreeBuilder` as its first argument; any remaining arguments become parameters of the factory. Calling it builds the graph and returns the finished `bpy.types.NodeTree` data-block:

```{python}
import nodebpy as nb


@nb.geometry_tree("Scattered Spheres")
def scattered_spheres(tree, count: int = 64, radius: float = 0.05):
    points = g.Points(count) >> g.SetPosition(offset=g.RandomValue.vector(min=-1))
    _ = (
        points
...
node_group = scattered_spheres(count=32, radius=0.1)
node_group
```

```{python}
# | echo: false
nb.TreeBuilder(node_group)
```

Because the result is an ordinary node group, it can be assigned straight to a modifier:
...
modifier.node_group = scattered_spheres()
```

Like `CustomGeometryGroup`, the group is built once and cached by name: if a node group with the decorator's name already exists in `bpy.data.node_groups`, calling the function returns it without rebuilding -- so repeated calls (or re-running the script) share one data-block instead of piling up `.001` copies. Note this means the function's arguments only take effect on the first call; to rebuild with different values, remove the existing group or use a different name.

The decorators accept the same options as `TreeBuilder.geometry()` / `.shader()` / `.compositor()`: the tree `name`, `collapse` to collapse all nodes, the `arrange` algorithm, and `fake_user` to protect the group from being purged on save.
