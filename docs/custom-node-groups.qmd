...
- The function receives the group's `TreeBuilder` as its first argument.
- Every other parameter becomes an interface input. The socket type comes from the annotation (`int`, `float`, `bool`, `str`, or any `InputGeometry` / `InputVector` / `InputColor` ... alias from `nodebpy.types`) and the default from the signature. Inside the function those parameters are the input *sockets*, ready to link.
...
The group's internals, showing the interface inputs derived from the signature:
