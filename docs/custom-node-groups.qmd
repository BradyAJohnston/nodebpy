...
class MyNode(NodeGroupBuilder):
    ...
    def _build_group(cls, tree):  # 4. Graph logic
        return (value + g.Value(1.0)) >> tree.ouptuts.socket_float()
```
...
class Jitter(builder.NodeGroupBuilder):
    ...
    def _build_group(cls, tree):
        geometry = tree.inputs.socket_geometry("Geometry")
        amount = tree.inputs.socket_float("Amount", 0.2)
        seed = tree.inputs.socket_integer("Seed")

        offset = g.RandomValue.vector(min=-1, seed=seed) * amount
        result = geometry >> g.SetPosition(offset=offset)

        _ = result >> tree.outputs.socket_geometry()
```
...
class RadialArray(builder.NodeGroupBuilder):
    ...
    @classmethod
    def _build_group(cls, tree):
        geometry = tree.inputs.socket_geometry()
        count = tree.inputs.socket_integer("Count", 6)
        radius = tree.inputs.socket_float("Radius", 2.0)
...
with TreeBuilder("JitteredFlower") as tree:
    petals = tree.inputs.socket_integer("Petals", 20, min_value=3, max_value=24)
    radius = tree.inputs.socket_float("Radius", 0.5, min_value=0.1)
    jitter = tree.inputs.socket_float("Jitter", 0.1)
    out = tree.outputs.socket_geometry()
