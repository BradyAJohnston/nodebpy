...
|   | approach |   | Node Editors | PyPI |
|:--------------|:--------------|---------------|---------------|:-------------:|
| [TreeClipper](https://github.com/Algebraic-UG/tree_clipper) | JSON Export and Import | Nodes \<-\> `.json` | Geometry, Compositor, Shader | ✓ |
| [NodeToPython](https://github.com/BrendanParmer/NodeToPython) | Export `bpy` python script | Nodes -\> `.py` | Geoemtry, Compositor, Shader | — |
| [geometry-script](https://github.com/carson-katri/geometry-script) | decorated functions, method chaining | `.py` -\> Nodes | Geometry | — |
| [geonodes](https://github.com/al1brn/geonodes) | context-based, shared node wrappers, socket method chaining | `.py` -\> Nodes | Geometry, Shader | — |
...
> The code should give an intuitive sense for the layout of the tree; through type hinting, auto-complete and docstrings, as much should be knowable about the tree ahead of time as possible.
...
with g.tree() as tree:
    ...
    g.RandomValue.vector() * 0.1 >> sp.i.offset
...
## Sockets have convenience accessors
...
#| output-location: column
with g.tree() as tree:
    p = g.Position()  # the node
    ps = p.o.position  # the output `Position` socket

    ps.x - 10
    g.InstanceTransform().o.transform.rotation.rotate(None)

    ps.normalize().cross().distance().cos()
...
`array.py` is 858 lines.
PR #82 changed one keyword in it.
`python -m nodebpy.assets plot` rendered this headlessly.
...
::: notes
None of these is visible in a screenshot of the node editor unless you are looking at exactly the right node.
All of them are one grep away in the source.
:::
...
::: notes
Nobody opened a node editor to find these.
The constants were wrong in the source, and the numbers were checkable against the reference paper.
:::
