...
toc-depth: 1
resources:
  - images/talk/iso_google_keynote.mp4
  - bcon/animation.mp4
format:
  revealjs:
...
I present an elegant new python scripting API for creating, editing and comparing node trees.
Visual programming with nodes is excellent for iteration and great for artist-friendly workflows, but creating or tracking node trees with code has remained a verbose and difficult task.
To address these problems, I created a new Python package `nodebpy` for authoring, versioning and interacting with node trees.
Human-writeable code becomes nicely arranged node trees, and GUI-authored node trees become human-readable `.py` files easier collaboration and review.
With shorter & more logical scripts, LLMs are also able to read and write node trees more reliably and with less token usage.
I will show the design and thought process behind the creation of this pacakge, and how it is currently being used in different add-ons and how it could benefit others authoring add-ons, building pipelines and trying to version control node trees.
:::

::: {.content-visible unless-format="revealjs"}
This page is the written version of a talk about [`nodebpy`](https://github.com/BradyAJohnston/nodebpy), a Python package for building Blender node trees as code.
The slides are embedded below, or available directly at [this link](slides.html).
...
::: incremental
- Creating a node group with python requires _a lot_ of `bpy` code
...
::: notes
Set up the two existing options: bundled .blend assets (opaque, un-diffable) or raw bpy (verbose, fragile).
Both are painful to maintain across Blender versions.
:::

## Many attempts at solving this {.smaller .compact-table}
...
: {tbl-colwidths="[20,38,20,14,8]"}

::: {.content-visible unless-format="revealjs"}
Side-by-side reproductions of examples from each project's own documentation are in the [comparisons page](comparisons.qmd) of the docs.
The short version: the storage-oriented tools are great at persistence but not authoring; the authoring tools each made different API trade-offs, and `nodebpy` is the only one distributed on PyPI for use as a dependency in other projects.
:::
...
example_tree = TreeBuilder(nt, arrange=None)
example_tree
```
...
::: {.content-visible unless-format="revealjs"}
27 lines of plumbing become a single pipeline that reads in the same order the data flows.
Sockets are matched automatically, layout is automatic, and every name here is a typed Python identifier your IDE can autocomplete and your type checker can verify.
:::
...
<video class="r-stretch" src="images/talk/iso_google_keynote.mp4" data-autoplay autoplay loop muted playsinline></video>
...
<video class="r-stretch" src="bcon/animation.mp4" data-autoplay autoplay loop muted playsinline></video>

## Giving Access to the Node API

![](bcon/animation_nodes_example.png)

## Giving Acces to the Node API {.smaller}

```{.python include="bcon/main.py"}
```
...
Comparing sockets builds a `Compare` node — evaluated *per frame, per element* in the tree.
Comparing plain Python values is evaluated *once*, at build time:
...
::: notes
Worth dwelling on for a technical audience — the operator overloading is doing graph construction, not evaluation.
Same mental model as any embedded DSL / expression-template library.
:::
...
```{python}
#| output-location: column
#| code-line-numbers: "|5-15|17-25|27-32|"
...
::: {.content-visible unless-format="revealjs"}
This is the migration path: wire a tree up by hand in the GUI where that's fastest, then `to_python()` it into version-controlled, reviewable, diffable source.
It also makes snapshot-testing node trees trivial.
:::
...
::: notes
This is PR #94, "Preserve Normals reverted to False for assets".
The whole story of that PR is on the next few slides, and none of it is visible here.
:::
...
## Eight real PRs, twice {.smaller .compact-table}
...
::: notes
Dumped with the 5.3 branch of nodebpy on the bpy 5.3 alpha wheel.
Row #91 is the one to watch: a PR about one asset that changes four files.
:::
...
The binary workflow gives a reviewer no way to see that a change to one asset carried three unrelated reverts.
:::
...
- Agents can now read and write node trees with `nodebpy` (as it's _just_ python)
...
## `blender/blender-assets` {.smaller}
...
- ![](images/paste-4.png){width=300px}
...
344 node modules are the source of truth; `nodes.blend` is a build product.
`check` in CI proves build → dump reproduces them exactly.
...
{{< qrcode https://github.com/BradyAJohnston/nodebpy width=300 height=300 >}}