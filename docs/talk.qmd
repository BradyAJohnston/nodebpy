...
with g.tree() as tree:
    ...
    comp = frame > 50  # Compare[Float].o.result

    (
        comp.switch.geometry(g.Cube(), g.Cone())
        >> tree.outputs.geometry()
    )
...
Remember the raw `bpy` tree from the start?
Any tree — GUI-built, appended, or raw `bpy` — converts back to `nodebpy` source:
