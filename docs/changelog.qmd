...
- **`Default` spells an input's fallback** — Blender lets an input read an implicit field, a context value or a named attribute when nothing is connected (a group interface's *Default Input* / *Default Attribute*, or a built-in such as Set Position's *Position*), hiding the stored value. `nodebpy.Default` mirrors the default-input identifiers (`Default.POSITION`, `Default.ID_OR_INDEX`, `Default.SELF_OBJECT`, …) and `Default.attribute("UVMap")` names an attribute. Generated asset classes use them as parameter defaults (`id: InputInteger = Default.ID_OR_INDEX`) and say in the docstring what the input reads when unconnected; passing a member to any node constructor leaves the socket untouched, like `None`, and `to_python` treats such a default as "nothing to export". The `tree.inputs.*` / `tree.outputs.*` factories accept the members for `default_input=` and `default_attribute=` alongside the identifier strings. Built-in node inputs whose value Blender hides (`hide_value`: Set Position's *Position*, every *Selection*, …) now default to `None` instead of advertising a value Blender never shows.
