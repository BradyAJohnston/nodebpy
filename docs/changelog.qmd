...
- Domain aggregation properties on `FloatSocket`, `VectorSocket`, and `IntegerSocket` — compute field statistics per geometry domain. Each returns a typed `NamedTuple`.
    - `.min_max.point()` / `.edge()` / `.face()` / `.corner()` / `.spline()` / `.instance()` / `.layer()` → `MinMaxReturn(min, max)`
    - `.average.point()` / … → `AverageFieldResult(mean, median)` *(Float and Vector only)*
    - `.variance.point()` / … → `FieldVarianceResult(standard_deviation, variance)` *(Float and Vector only)*

```py
lo, hi = position.min_max.point()
mean, median = curvature.average.face()
std_dev, variance = weight.variance.point(group_index)
```
