...
- Domain aggregation on `FloatSocket`, `VectorSocket`, and `IntegerSocket` — select a domain property, then call the statistic. Each call returns a single socket.
    - Domain properties: `.point`, `.edge`, `.face`, `.corner`, `.spline`, `.instance`, `.layer`
    - Float/Vector statistics: `.min()`, `.max()`, `.mean()`, `.median()`, `.std_dev()`, `.variance()`
    - Integer statistics: `.min()`, `.max()`
    - All domain methods accept an optional `group_index` argument

```py
lo = position.point.min()              # VectorSocket
hi = position.edge.max()               # VectorSocket
mean = curvature.face.mean()           # FloatSocket
std_dev = weight.point.std_dev(group_index)  # FloatSocket
```
