...
### Enhancements
- Tree interfaces are defined with `tree.inputs.geometry()` methods rather than using the old context-based `s.SocketGeometry()`. Both systems have been living side-by-side but this completely removed old system so is a hard breaking change. (See [#67](https://github.com/BradyAJohnston/nodebpy/pull/67))
```py
# old system
with tree.inputs:
    s.SocketGeometry()
