...
```{python}
with g.tree("NewTree") as tree:
    geom_inputs = [tree.inputs.socket_geometry(f"Geometry_{i}") for i in range(5)]
    g.JoinGeometry(*geom_inputs) >> tree.outputs.socket_geometry("The Output Socket")
...
with g.tree() as tree:
    (
        tree.inputs.socket_integer("Count", 10)
...
with g.tree() as tree:
    ...
    output = tree.outputs.socket_geometry()
