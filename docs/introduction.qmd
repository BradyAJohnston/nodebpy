...
We can access the input and output nodes with `zone.input` and `zone.output`. The repeat zone as the `zone.i` which is the iteration number of the current zone. Simulation zone has the `zone.output.o.delta_time` which is the time between previous and current simulation loop.
