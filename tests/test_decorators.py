...
def scatter(
    tree, geometry: InputGeometry, count: InputInteger = 64, radius: float = 0.05
):
    """Scatter spheres on the input geometry."""
    points = geometry >> g.DistributePointsOnFaces(density=count)
    _ = (
        points
        >> g.InstanceOnPoints(instance=g.IcoSphere(radius=radius))
...
def test_signature_becomes_interface():
    ...
    assert list(scatter.__signature__.parameters) == ["geometry", "count", "radius"]
...
def test_name_defaults_to_function_name():
    @nb.geometry_tree()
    def offset_points(tree, offset: float = 1.0):
        _ = g.Points(1) >> g.SetPosition(offset=offset) >> tree.outputs.geometry()
...
def test_string_annotations():
    ...
    def stringy(tree, count: "int" = 3, scale: "nb.types.InputFloat" = 2.0):
        _ = g.Points(count) >> tree.outputs.geometry()
        _ = scale >> tree.outputs.float()
...
def test_shader_tree():
    ...
    def glossy(tree, roughness: InputFloat = 0.25):
...
def test_compositor_tree():
    ...
    def blur(tree, image: nb.types.InputColor, size: float = 4.0):
