...
def _format_key_differences(differences: dict, expected_data: dict, current_data: dict) -> str:
    ...
    expected_node_sigs = [get_node_signature(node.get("data", {})) for node in expected_nodes]
    current_node_sigs = [get_node_signature(node.get("data", {})) for node in current_nodes]
    ...
    if len(expected_nodes) != len(current_nodes):
        summary.append(f"• Node count: expected {len(expected_nodes)}, got {len(current_nodes)}")
    ...
    expected_link_sigs = [get_link_signature(link.get("data", {}), expected_nodes) for link in expected_links]
    current_link_sigs = [get_link_signature(link.get("data", {}), current_nodes) for link in current_links]
    ...
    if len(expected_links) != len(current_links):
        summary.append(f"• Link count: expected {len(expected_links)}, got {len(current_links)}")
    
    # Check for property changes in nodes when counts are the same
    if len(expected_nodes) == len(current_nodes) and len(new_nodes) == 0 and len(missing_nodes) == 0:
        property_changes = []
        ...
        for i, (exp_node, curr_node) in enumerate(zip(expected_nodes, current_nodes)):
            ...
            exp_interface = exp_data.get("interface", {}).get("data", {}).get("items_tree", {}).get("data", {}).get("items", [])
            curr_interface = curr_data.get("interface", {}).get("data", {}).get("items_tree", {}).get("data", {}).get("items", [])
            
            for j, (exp_sock, curr_sock) in enumerate(zip(exp_interface, curr_interface)):
                exp_sock_data = exp_sock.get("data", {})
                curr_sock_data = curr_sock.get("data", {})
                
                if exp_sock_data.get("default_value") != curr_sock_data.get("default_value"):
                    ...
                    property_changes.append(f"interface socket '{sock_name}': {exp_val} → {curr_val}")
            ...
            for j, (exp_input, curr_input) in enumerate(zip(exp_inputs, curr_inputs)):
                ...
                if exp_input_data.get("default_value") != curr_input_data.get("default_value"):
                    ...
                    property_changes.append(f"{node_name}.{input_name}: {exp_val} → {curr_val}")
            
            # Check other node properties
            for prop in ["operation", "use_clamp", "data_type"]:
                if prop in exp_data and prop in curr_data and exp_data[prop] != curr_data[prop]:
                    property_changes.append(f"{node_name}.{prop}: {exp_data[prop]} → {curr_data[prop]}")
        
        if property_changes:
            summary.append(f"• Property changes: {', '.join(property_changes)}")
    
    # Check for interface differences
    expected_interface = expected_tree.get("interface", {}).get("data", {}).get("items_tree", {}).get("data", {}).get("items", [])
    current_interface = current_tree.get("interface", {}).get("data", {}).get("items_tree", {}).get("data", {}).get("items", [])
    
    if len(expected_interface) != len(current_interface):
        summary.append(f"• Interface socket count: expected {len(expected_interface)}, got {len(current_interface)}")
    
    # Check for interface property changes
    if len(expected_interface) == len(current_interface):
        interface_changes = []
        for i, (exp_sock, curr_sock) in enumerate(zip(expected_interface, current_interface)):
    ...
    def matches(
        ...
        try:
            ...
            try:
                ...
                print(f"Expected tree name: {expected_data.get('node_trees', [{}])[0].get('data', {}).get('name', 'Unknown')}")
                print(f"Actual tree name: {current_data.get('node_trees', [{}])[0].get('data', {}).get('name', 'Unknown')}")
