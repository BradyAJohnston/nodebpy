...
- [Writing Node Trees](introduction.llms.md) — adding and linking nodes, tree contexts, interface sockets and zones
- [Math Operators](operators.llms.md) — arithmetic, comparison and boolean expressions as nodes
- [Node API Design](node-api.llms.md) — sockets, the `.i` / `.o` accessors, enum options and convenience class methods
- [Custom Node Groups](custom-node-groups.llms.md) — encapsulate reusable logic as Python classes
- [Asset Node Groups](assets.llms.md) — typed classes generated from `.blend` asset libraries
- [Nodes to Code](nodes-to-code.llms.md) — turn existing node trees back into `nodebpy` source
- [Using `nodebpy`](using.llms.md) — shipping it inside your own add-ons, and the versioning scheme
- [Comparisons](comparisons.llms.md) — related projects and how the APIs differ
...
The top priority of `nodebpy` has been type hinting and IDE auto-complete: any tooling that supports authoring regular Python code should also support authoring node trees. Much like [`databpy`](https://github.com/BradyAJohnston/databpy), this started as an internal tool used inside of [`molecularnodes`](https://github.com/BradyAJohnston/molecularnodes) but has since been broken out into its own separate project. It is robustly typed and tested, with the intent that it can be used internally for multiple other add-ons and projects.
