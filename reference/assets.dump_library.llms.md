...
| names | set\[str\] \| None | Restrict the dump to these asset (node-group or material) names; defaults to all. A full dump first clears the managed subdirectories (`geometry`/`shader`/`compositor`/`materials`) so files from renamed or deleted assets don’t linger; a filtered dump leaves the other assets’ files in place. | `None` |
