...
# While > 0, asset groups that carry their own ``_build_group`` recipe build
# their tree from that source instead of appending the ``.blend`` — see
# :func:`build_from_source`.
_build_from_source_depth = 0


@contextmanager
def build_from_source():
    """Make asset groups build from their ``_build_group`` source.

    A merged asset class (generated by ``dump_library(..., typed_api=True)``)
    both appends its group from the shipped ``.blend`` *and* carries the
    ``_build_group`` recipe that regenerates it. Inside this context,
    ``create_group()`` runs the recipe instead of appending — this is how
    :func:`nodebpy.assets.build_library` rebuilds the ``.blend`` from source.
    Classes without a recipe (plain append-only asset classes) are unaffected.
    """
    global _build_from_source_depth
    _build_from_source_depth += 1
    try:
        yield
    finally:
        _build_from_source_depth -= 1
...
class _AssetGroupMixin:
    ...
    @classmethod
    def _has_build_source(cls) -> bool:
        """Whether this class overrides ``_build_group`` with a real recipe
    ...
    def create_group(cls):
        ...
        A class that also carries a ``_build_group`` recipe builds the tree
        from that source instead — inside a :func:`build_from_source` context
        (how ``build_library`` regenerates the ``.blend``), or as a fallback
