# Auto-generated by generate.py — do not edit manually.

from typing import Literal

import bpy

from ...builder import (
    BaseNode as NodeBuilder,
    SocketAccessor,
    Socket,
    BooleanSocket,
    BundleSocket,
    ColorSocket,
    FloatSocket,
    GeometrySocket,
    IntegerSocket,
    MatrixSocket,
    RotationSocket,
    StringSocket,
    VectorSocket,
)

from ...types import (
    InputBoolean,
    InputBundle,
    InputClosure,
    InputCollection,
    InputColor,
    InputGeometry,
    InputImage,
    InputInteger,
    InputMaterial,
    InputMatrix,
    InputMenu,
    InputObject,
    InputRotation,
    InputString,
    InputFloat,
    InputVector,
)


class AlignRotationToVector(NodeBuilder):
    """
    Orient a rotation along the given direction

    Parameters
    ----------
    rotation : InputRotation
        Rotation
    factor : InputFloat
        Factor
    vector : InputVector
        Vector

    Outputs
    -------
    rotation : RotationSocket
        Rotation
    """

    _bl_idname = "FunctionNodeAlignRotationToVector"
    node: bpy.types.FunctionNodeAlignRotationToVector

    class Inputs(SocketAccessor):
        rotation: Socket
        factor: Socket
        vector: Socket

    class Outputs(SocketAccessor):
        rotation: RotationSocket

    @property
    def i(self) -> "AlignRotationToVector.Inputs":
        return AlignRotationToVector.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "AlignRotationToVector.Outputs":
        return AlignRotationToVector.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        rotation: InputRotation = None,
        factor: InputFloat = 1.0,
        vector: InputVector = None,
        *,
        axis: Literal["X", "Y", "Z"] = "Z",
        pivot_axis: Literal["AUTO", "X", "Y", "Z"] = "AUTO",
    ):
        super().__init__()
        key_args = {"Rotation": rotation, "Factor": factor, "Vector": vector}
        self.axis = axis
        self.pivot_axis = pivot_axis
        self._establish_links(**key_args)

    @property
    def axis(self) -> Literal["X", "Y", "Z"]:
        return self.node.axis

    @axis.setter
    def axis(self, value: Literal["X", "Y", "Z"]):
        self.node.axis = value

    @property
    def pivot_axis(self) -> Literal["AUTO", "X", "Y", "Z"]:
        return self.node.pivot_axis

    @pivot_axis.setter
    def pivot_axis(self, value: Literal["AUTO", "X", "Y", "Z"]):
        self.node.pivot_axis = value


class AxesToRotation(NodeBuilder):
    """
    Create a rotation from a primary and (ideally orthogonal) secondary axis

    Parameters
    ----------
    primary_axis : InputVector
        Primary Axis
    secondary_axis : InputVector
        Secondary Axis

    Outputs
    -------
    rotation : RotationSocket
        Rotation
    """

    _bl_idname = "FunctionNodeAxesToRotation"
    node: bpy.types.FunctionNodeAxesToRotation

    class Inputs(SocketAccessor):
        primary_axis: Socket
        secondary_axis: Socket

    class Outputs(SocketAccessor):
        rotation: RotationSocket

    @property
    def i(self) -> "AxesToRotation.Inputs":
        return AxesToRotation.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "AxesToRotation.Outputs":
        return AxesToRotation.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        primary_axis: InputVector = None,
        secondary_axis: InputVector = None,
        *,
        primary: Literal["X", "Y", "Z"] = "Z",
        secondary: Literal["X", "Y", "Z"] = "X",
    ):
        super().__init__()
        key_args = {"Primary Axis": primary_axis, "Secondary Axis": secondary_axis}
        self.primary_axis = primary_axis
        self.secondary_axis = secondary_axis
        self._establish_links(**key_args)

    @property
    def primary(self) -> Literal["X", "Y", "Z"]:
        return self.node.primary_axis

    @primary.setter
    def primary(self, value: Literal["X", "Y", "Z"]):
        self.node.primary_axis = value

    @property
    def secondary(self) -> Literal["X", "Y", "Z"]:
        return self.node.secondary_axis

    @secondary.setter
    def secondary(self, value: Literal["X", "Y", "Z"]):
        self.node.secondary_axis = value


class AxisAngleToRotation(NodeBuilder):
    """
    Build a rotation from an axis and a rotation around that axis

    Parameters
    ----------
    axis : InputVector
        Axis
    angle : InputFloat
        Angle

    Outputs
    -------
    rotation : RotationSocket
        Rotation
    """

    _bl_idname = "FunctionNodeAxisAngleToRotation"
    node: bpy.types.FunctionNodeAxisAngleToRotation

    class Inputs(SocketAccessor):
        axis: Socket
        angle: Socket

    class Outputs(SocketAccessor):
        rotation: RotationSocket

    @property
    def i(self) -> "AxisAngleToRotation.Inputs":
        return AxisAngleToRotation.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "AxisAngleToRotation.Outputs":
        return AxisAngleToRotation.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        axis: InputVector = None,
        angle: InputFloat = 0.0,
    ):
        super().__init__()
        key_args = {"Axis": axis, "Angle": angle}

        self._establish_links(**key_args)


class BitMath(NodeBuilder):
    """
    Perform bitwise operations on 32-bit integers

    Parameters
    ----------
    a : InputInteger
        A
    b : InputInteger
        B
    shift : InputInteger
        Shift

    Outputs
    -------
    value : IntegerSocket
        Value
    """

    _bl_idname = "FunctionNodeBitMath"
    node: bpy.types.FunctionNodeBitMath

    class Inputs(SocketAccessor):
        a: Socket
        b: Socket
        shift: Socket

    class Outputs(SocketAccessor):
        value: IntegerSocket

    @property
    def i(self) -> "BitMath.Inputs":
        return BitMath.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "BitMath.Outputs":
        return BitMath.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        a: InputInteger = 0,
        b: InputInteger = 0,
        shift: InputInteger = 0,
        *,
        operation: Literal["AND", "OR", "XOR", "NOT", "SHIFT", "ROTATE"] = "AND",
    ):
        super().__init__()
        key_args = {"A": a, "B": b, "Shift": shift}
        self.operation = operation
        self._establish_links(**key_args)

    @classmethod
    def l_and(cls, a: InputInteger = 0, b: InputInteger = 0) -> "BitMath":
        """Create Bit Math with operation 'And'. Returns a value where the bits of A and B are both set"""
        return cls(operation="AND", a=a, b=b)

    @classmethod
    def l_or(cls, a: InputInteger = 0, b: InputInteger = 0) -> "BitMath":
        """Create Bit Math with operation 'Or'. Returns a value where the bits of either A or B are set"""
        return cls(operation="OR", a=a, b=b)

    @classmethod
    def exclusive_or(cls, a: InputInteger = 0, b: InputInteger = 0) -> "BitMath":
        """Create Bit Math with operation 'Exclusive Or'. Returns a value where only one bit from A and B is set"""
        return cls(operation="XOR", a=a, b=b)

    @classmethod
    def l_not(cls, a: InputInteger = 0) -> "BitMath":
        """Create Bit Math with operation 'Not'. Returns the opposite bit value of A, in decimal it is equivalent of A = -A - 1"""
        return cls(operation="NOT", a=a)

    @classmethod
    def shift(cls, a: InputInteger = 0, shift: InputInteger = 0) -> "BitMath":
        """Create Bit Math with operation 'Shift'. Shifts the bit values of A by the specified Shift amount. Positive values shift left, negative values shift right."""
        return cls(operation="SHIFT", a=a, shift=shift)

    @classmethod
    def rotate(cls, a: InputInteger = 0, shift: InputInteger = 0) -> "BitMath":
        """Create Bit Math with operation 'Rotate'. Rotates the bit values of A by the specified Shift amount. Positive values rotate left, negative values rotate right."""
        return cls(operation="ROTATE", a=a, shift=shift)

    @property
    def operation(self) -> Literal["AND", "OR", "XOR", "NOT", "SHIFT", "ROTATE"]:
        return self.node.operation

    @operation.setter
    def operation(self, value: Literal["AND", "OR", "XOR", "NOT", "SHIFT", "ROTATE"]):
        self.node.operation = value


class Blackbody(NodeBuilder):
    """
    Convert a blackbody temperature to an RGB value

    Parameters
    ----------
    temperature : InputFloat
        Temperature

    Outputs
    -------
    color : ColorSocket
        Color
    """

    _bl_idname = "ShaderNodeBlackbody"
    node: bpy.types.ShaderNodeBlackbody

    class Inputs(SocketAccessor):
        temperature: Socket

    class Outputs(SocketAccessor):
        color: ColorSocket

    @property
    def i(self) -> "Blackbody.Inputs":
        return Blackbody.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "Blackbody.Outputs":
        return Blackbody.Outputs(self.node.outputs, "output")

    def __init__(self, temperature: InputFloat = 6500.0):
        super().__init__()
        key_args = {"Temperature": temperature}

        self._establish_links(**key_args)


class BooleanMath(NodeBuilder):
    """
    Perform a logical operation on the given boolean inputs

    Parameters
    ----------
    boolean : InputBoolean
        Boolean
    boolean_001 : InputBoolean
        Boolean

    Outputs
    -------
    boolean : BooleanSocket
        Boolean
    """

    _bl_idname = "FunctionNodeBooleanMath"
    node: bpy.types.FunctionNodeBooleanMath

    class Inputs(SocketAccessor):
        boolean: Socket
        boolean_001: Socket

    class Outputs(SocketAccessor):
        boolean: BooleanSocket

    @property
    def i(self) -> "BooleanMath.Inputs":
        return BooleanMath.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "BooleanMath.Outputs":
        return BooleanMath.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        boolean: InputBoolean = False,
        boolean_001: InputBoolean = False,
        *,
        operation: Literal[
            "AND", "OR", "NOT", "NAND", "NOR", "XNOR", "XOR", "IMPLY", "NIMPLY"
        ] = "AND",
    ):
        super().__init__()
        key_args = {"Boolean": boolean, "Boolean_001": boolean_001}
        self.operation = operation
        self._establish_links(**key_args)

    @classmethod
    def l_and(
        cls, boolean: InputBoolean = False, boolean_001: InputBoolean = False
    ) -> "BooleanMath":
        """Create Boolean Math with operation 'And'. True when both inputs are true"""
        return cls(operation="AND", boolean=boolean, boolean_001=boolean_001)

    @classmethod
    def l_or(
        cls, boolean: InputBoolean = False, boolean_001: InputBoolean = False
    ) -> "BooleanMath":
        """Create Boolean Math with operation 'Or'. True when at least one input is true"""
        return cls(operation="OR", boolean=boolean, boolean_001=boolean_001)

    @classmethod
    def l_not(cls, boolean: InputBoolean = False) -> "BooleanMath":
        """Create Boolean Math with operation 'Not'. Opposite of the input"""
        return cls(operation="NOT", boolean=boolean)

    @classmethod
    def not_and(
        cls, boolean: InputBoolean = False, boolean_001: InputBoolean = False
    ) -> "BooleanMath":
        """Create Boolean Math with operation 'Not And'. True when at least one input is false"""
        return cls(operation="NAND", boolean=boolean, boolean_001=boolean_001)

    @classmethod
    def nor(
        cls, boolean: InputBoolean = False, boolean_001: InputBoolean = False
    ) -> "BooleanMath":
        """Create Boolean Math with operation 'Nor'. True when both inputs are false"""
        return cls(operation="NOR", boolean=boolean, boolean_001=boolean_001)

    @classmethod
    def equal(
        cls, boolean: InputBoolean = False, boolean_001: InputBoolean = False
    ) -> "BooleanMath":
        """Create Boolean Math with operation 'Equal'. True when both inputs are equal (exclusive nor)"""
        return cls(operation="XNOR", boolean=boolean, boolean_001=boolean_001)

    @classmethod
    def not_equal(
        cls, boolean: InputBoolean = False, boolean_001: InputBoolean = False
    ) -> "BooleanMath":
        """Create Boolean Math with operation 'Not Equal'. True when both inputs are different (exclusive or)"""
        return cls(operation="XOR", boolean=boolean, boolean_001=boolean_001)

    @classmethod
    def imply(
        cls, boolean: InputBoolean = False, boolean_001: InputBoolean = False
    ) -> "BooleanMath":
        """Create Boolean Math with operation 'Imply'. True unless the first input is true and the second is false"""
        return cls(operation="IMPLY", boolean=boolean, boolean_001=boolean_001)

    @classmethod
    def subtract(
        cls, boolean: InputBoolean = False, boolean_001: InputBoolean = False
    ) -> "BooleanMath":
        """Create Boolean Math with operation 'Subtract'. True when the first input is true and the second is false (not imply)"""
        return cls(operation="NIMPLY", boolean=boolean, boolean_001=boolean_001)

    @property
    def operation(
        self,
    ) -> Literal["AND", "OR", "NOT", "NAND", "NOR", "XNOR", "XOR", "IMPLY", "NIMPLY"]:
        return self.node.operation

    @operation.setter
    def operation(
        self,
        value: Literal[
            "AND", "OR", "NOT", "NAND", "NOR", "XNOR", "XOR", "IMPLY", "NIMPLY"
        ],
    ):
        self.node.operation = value


class Clamp(NodeBuilder):
    """
    Clamp a value between a minimum and a maximum

    Parameters
    ----------
    value : InputFloat
        Value
    min : InputFloat
        Min
    max : InputFloat
        Max

    Outputs
    -------
    result : FloatSocket
        Result
    """

    _bl_idname = "ShaderNodeClamp"
    node: bpy.types.ShaderNodeClamp

    class Inputs(SocketAccessor):
        value: Socket
        min: Socket
        max: Socket

    class Outputs(SocketAccessor):
        result: FloatSocket

    @property
    def i(self) -> "Clamp.Inputs":
        return Clamp.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "Clamp.Outputs":
        return Clamp.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        value: InputFloat = 1.0,
        min: InputFloat = 0.0,
        max: InputFloat = 1.0,
        *,
        clamp_type: Literal["MINMAX", "RANGE"] = "MINMAX",
    ):
        super().__init__()
        key_args = {"Value": value, "Min": min, "Max": max}
        self.clamp_type = clamp_type
        self._establish_links(**key_args)

    @classmethod
    def min_max(
        cls, value: InputFloat = 1.0, min: InputFloat = 0.0, max: InputFloat = 1.0
    ) -> "Clamp":
        """Create Clamp with operation 'Min Max'. Constrain value between min and max"""
        return cls(clamp_type="MINMAX", value=value, min=min, max=max)

    @classmethod
    def range(
        cls, value: InputFloat = 1.0, min: InputFloat = 0.0, max: InputFloat = 1.0
    ) -> "Clamp":
        """Create Clamp with operation 'Range'. Constrain value between min and max, swapping arguments when min > max"""
        return cls(clamp_type="RANGE", value=value, min=min, max=max)

    @property
    def clamp_type(self) -> Literal["MINMAX", "RANGE"]:
        return self.node.clamp_type

    @clamp_type.setter
    def clamp_type(self, value: Literal["MINMAX", "RANGE"]):
        self.node.clamp_type = value


class ColorRamp(NodeBuilder):
    """
    Map values to colors with the use of a gradient

    Parameters
    ----------
    fac : InputFloat
        Factor

    Outputs
    -------
    color : ColorSocket
        Color
    alpha : FloatSocket
        Alpha
    """

    _bl_idname = "ShaderNodeValToRGB"
    node: bpy.types.ShaderNodeValToRGB

    class Inputs(SocketAccessor):
        fac: Socket

    class Outputs(SocketAccessor):
        color: ColorSocket
        alpha: FloatSocket

    @property
    def i(self) -> "ColorRamp.Inputs":
        return ColorRamp.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "ColorRamp.Outputs":
        return ColorRamp.Outputs(self.node.outputs, "output")

    def __init__(self, fac: InputFloat = 0.5):
        super().__init__()
        key_args = {"Fac": fac}

        self._establish_links(**key_args)


class CombineBundle(NodeBuilder):
    """
    Combine multiple socket values into one.

    Outputs
    -------
    bundle : BundleSocket
        Bundle
    """

    _bl_idname = "NodeCombineBundle"
    node: bpy.types.Node

    class Inputs(SocketAccessor):
        pass

    class Outputs(SocketAccessor):
        bundle: BundleSocket

    @property
    def i(self) -> "CombineBundle.Inputs":
        return CombineBundle.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "CombineBundle.Outputs":
        return CombineBundle.Outputs(self.node.outputs, "output")

    def __init__(self, define_signature: bool = False):
        super().__init__()
        key_args = {}
        self.define_signature = define_signature
        self._establish_links(**key_args)

    @property
    def define_signature(self) -> bool:
        return self.node.define_signature

    @define_signature.setter
    def define_signature(self, value: bool):
        self.node.define_signature = value


class CombineColor(NodeBuilder):
    """
    Combine four channels into a single color, based on a particular color model

    Parameters
    ----------
    red : InputFloat
        Red
    green : InputFloat
        Green
    blue : InputFloat
        Blue
    alpha : InputFloat
        Alpha

    Outputs
    -------
    color : ColorSocket
        Color
    """

    _bl_idname = "FunctionNodeCombineColor"
    node: bpy.types.FunctionNodeCombineColor

    class Inputs(SocketAccessor):
        red: Socket
        green: Socket
        blue: Socket
        alpha: Socket

    class Outputs(SocketAccessor):
        color: ColorSocket

    @property
    def i(self) -> "CombineColor.Inputs":
        return CombineColor.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "CombineColor.Outputs":
        return CombineColor.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        red: InputFloat = 0.0,
        green: InputFloat = 0.0,
        blue: InputFloat = 0.0,
        alpha: InputFloat = 1.0,
        *,
        mode: Literal["RGB", "HSV", "HSL"] = "RGB",
    ):
        super().__init__()
        key_args = {"Red": red, "Green": green, "Blue": blue, "Alpha": alpha}
        self.mode = mode
        self._establish_links(**key_args)

    @classmethod
    def rgb(
        cls,
        red: InputFloat = 0.0,
        green: InputFloat = 0.0,
        blue: InputFloat = 0.0,
        alpha: InputFloat = 1.0,
    ) -> "CombineColor":
        """Create Combine Color with operation 'RGB'. Use RGB (Red, Green, Blue) color processing"""
        return cls(mode="RGB", red=red, green=green, blue=blue, alpha=alpha)

    @classmethod
    def hsv(
        cls,
        red: InputFloat = 0.0,
        green: InputFloat = 0.0,
        blue: InputFloat = 0.0,
        alpha: InputFloat = 1.0,
    ) -> "CombineColor":
        """Create Combine Color with operation 'HSV'. Use HSV (Hue, Saturation, Value) color processing"""
        return cls(mode="HSV", red=red, green=green, blue=blue, alpha=alpha)

    @classmethod
    def hsl(
        cls,
        red: InputFloat = 0.0,
        green: InputFloat = 0.0,
        blue: InputFloat = 0.0,
        alpha: InputFloat = 1.0,
    ) -> "CombineColor":
        """Create Combine Color with operation 'HSL'. Use HSL (Hue, Saturation, Lightness) color processing"""
        return cls(mode="HSL", red=red, green=green, blue=blue, alpha=alpha)

    @property
    def mode(self) -> Literal["RGB", "HSV", "HSL"]:
        return self.node.mode

    @mode.setter
    def mode(self, value: Literal["RGB", "HSV", "HSL"]):
        self.node.mode = value


class CombineMatrix(NodeBuilder):
    """
    Construct a 4x4 matrix from its individual values

    Parameters
    ----------
    column_1_row_1 : InputFloat
        Column 1 Row 1
    column_1_row_2 : InputFloat
        Column 1 Row 2
    column_1_row_3 : InputFloat
        Column 1 Row 3
    column_1_row_4 : InputFloat
        Column 1 Row 4
    column_2_row_1 : InputFloat
        Column 2 Row 1
    column_2_row_2 : InputFloat
        Column 2 Row 2
    column_2_row_3 : InputFloat
        Column 2 Row 3
    column_2_row_4 : InputFloat
        Column 2 Row 4
    column_3_row_1 : InputFloat
        Column 3 Row 1
    column_3_row_2 : InputFloat
        Column 3 Row 2
    column_3_row_3 : InputFloat
        Column 3 Row 3
    column_3_row_4 : InputFloat
        Column 3 Row 4
    column_4_row_1 : InputFloat
        Column 4 Row 1
    column_4_row_2 : InputFloat
        Column 4 Row 2
    column_4_row_3 : InputFloat
        Column 4 Row 3
    column_4_row_4 : InputFloat
        Column 4 Row 4

    Outputs
    -------
    matrix : MatrixSocket
        Matrix
    """

    _bl_idname = "FunctionNodeCombineMatrix"
    node: bpy.types.FunctionNodeCombineMatrix

    class Inputs(SocketAccessor):
        column_1_row_1: Socket
        column_1_row_2: Socket
        column_1_row_3: Socket
        column_1_row_4: Socket
        column_2_row_1: Socket
        column_2_row_2: Socket
        column_2_row_3: Socket
        column_2_row_4: Socket
        column_3_row_1: Socket
        column_3_row_2: Socket
        column_3_row_3: Socket
        column_3_row_4: Socket
        column_4_row_1: Socket
        column_4_row_2: Socket
        column_4_row_3: Socket
        column_4_row_4: Socket

    class Outputs(SocketAccessor):
        matrix: MatrixSocket

    @property
    def i(self) -> "CombineMatrix.Inputs":
        return CombineMatrix.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "CombineMatrix.Outputs":
        return CombineMatrix.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        column_1_row_1: InputFloat = 1.0,
        column_1_row_2: InputFloat = 0.0,
        column_1_row_3: InputFloat = 0.0,
        column_1_row_4: InputFloat = 0.0,
        column_2_row_1: InputFloat = 0.0,
        column_2_row_2: InputFloat = 1.0,
        column_2_row_3: InputFloat = 0.0,
        column_2_row_4: InputFloat = 0.0,
        column_3_row_1: InputFloat = 0.0,
        column_3_row_2: InputFloat = 0.0,
        column_3_row_3: InputFloat = 1.0,
        column_3_row_4: InputFloat = 0.0,
        column_4_row_1: InputFloat = 0.0,
        column_4_row_2: InputFloat = 0.0,
        column_4_row_3: InputFloat = 0.0,
        column_4_row_4: InputFloat = 1.0,
    ):
        super().__init__()
        key_args = {
            "Column 1 Row 1": column_1_row_1,
            "Column 1 Row 2": column_1_row_2,
            "Column 1 Row 3": column_1_row_3,
            "Column 1 Row 4": column_1_row_4,
            "Column 2 Row 1": column_2_row_1,
            "Column 2 Row 2": column_2_row_2,
            "Column 2 Row 3": column_2_row_3,
            "Column 2 Row 4": column_2_row_4,
            "Column 3 Row 1": column_3_row_1,
            "Column 3 Row 2": column_3_row_2,
            "Column 3 Row 3": column_3_row_3,
            "Column 3 Row 4": column_3_row_4,
            "Column 4 Row 1": column_4_row_1,
            "Column 4 Row 2": column_4_row_2,
            "Column 4 Row 3": column_4_row_3,
            "Column 4 Row 4": column_4_row_4,
        }

        self._establish_links(**key_args)


class CombineTransform(NodeBuilder):
    """
    Combine a translation vector, a rotation, and a scale vector into a transformation matrix

    Parameters
    ----------
    translation : InputVector
        Translation
    rotation : InputRotation
        Rotation
    scale : InputVector
        Scale

    Outputs
    -------
    transform : MatrixSocket
        Transform
    """

    _bl_idname = "FunctionNodeCombineTransform"
    node: bpy.types.FunctionNodeCombineTransform

    class Inputs(SocketAccessor):
        translation: Socket
        rotation: Socket
        scale: Socket

    class Outputs(SocketAccessor):
        transform: MatrixSocket

    @property
    def i(self) -> "CombineTransform.Inputs":
        return CombineTransform.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "CombineTransform.Outputs":
        return CombineTransform.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        translation: InputVector = None,
        rotation: InputRotation = None,
        scale: InputVector = None,
    ):
        super().__init__()
        key_args = {"Translation": translation, "Rotation": rotation, "Scale": scale}

        self._establish_links(**key_args)


class CombineXYZ(NodeBuilder):
    """
    Create a vector from X, Y, and Z components

    Parameters
    ----------
    x : InputFloat
        X
    y : InputFloat
        Y
    z : InputFloat
        Z

    Outputs
    -------
    vector : VectorSocket
        Vector
    """

    _bl_idname = "ShaderNodeCombineXYZ"
    node: bpy.types.ShaderNodeCombineXYZ

    class Inputs(SocketAccessor):
        x: Socket
        y: Socket
        z: Socket

    class Outputs(SocketAccessor):
        vector: VectorSocket

    @property
    def i(self) -> "CombineXYZ.Inputs":
        return CombineXYZ.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "CombineXYZ.Outputs":
        return CombineXYZ.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        x: InputFloat = 0.0,
        y: InputFloat = 0.0,
        z: InputFloat = 0.0,
    ):
        super().__init__()
        key_args = {"X": x, "Y": y, "Z": z}

        self._establish_links(**key_args)


class EulerToRotation(NodeBuilder):
    """
    Build a rotation from separate angles around each axis

    Parameters
    ----------
    euler : InputVector
        Euler

    Outputs
    -------
    rotation : RotationSocket
        Rotation
    """

    _bl_idname = "FunctionNodeEulerToRotation"
    node: bpy.types.FunctionNodeEulerToRotation

    class Inputs(SocketAccessor):
        euler: Socket

    class Outputs(SocketAccessor):
        rotation: RotationSocket

    @property
    def i(self) -> "EulerToRotation.Inputs":
        return EulerToRotation.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "EulerToRotation.Outputs":
        return EulerToRotation.Outputs(self.node.outputs, "output")

    def __init__(self, euler: InputVector = None):
        super().__init__()
        key_args = {"Euler": euler}

        self._establish_links(**key_args)


class FindInString(NodeBuilder):
    """
    Find the number of times a given string occurs in another string and the position of the first match

    Parameters
    ----------
    string : InputString
        String
    search : InputString
        Search

    Outputs
    -------
    first_found : IntegerSocket
        First Found
    count : IntegerSocket
        Count
    """

    _bl_idname = "FunctionNodeFindInString"
    node: bpy.types.FunctionNodeFindInString

    class Inputs(SocketAccessor):
        string: Socket
        search: Socket

    class Outputs(SocketAccessor):
        first_found: IntegerSocket
        count: IntegerSocket

    @property
    def i(self) -> "FindInString.Inputs":
        return FindInString.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "FindInString.Outputs":
        return FindInString.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        string: InputString = "",
        search: InputString = "",
    ):
        super().__init__()
        key_args = {"String": string, "Search": search}

        self._establish_links(**key_args)


class FloatCurve(NodeBuilder):
    """
    Map an input float to a curve and outputs a float value

    Parameters
    ----------
    factor : InputFloat
        Factor
    value : InputFloat
        Value

    Outputs
    -------
    value : FloatSocket
        Value
    """

    _bl_idname = "ShaderNodeFloatCurve"
    node: bpy.types.ShaderNodeFloatCurve

    class Inputs(SocketAccessor):
        factor: Socket
        value: Socket

    class Outputs(SocketAccessor):
        value: FloatSocket

    @property
    def i(self) -> "FloatCurve.Inputs":
        return FloatCurve.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "FloatCurve.Outputs":
        return FloatCurve.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        factor: InputFloat = 1.0,
        value: InputFloat = 1.0,
    ):
        super().__init__()
        key_args = {"Factor": factor, "Value": value}

        self._establish_links(**key_args)


class FloatToInteger(NodeBuilder):
    """
    Convert the given floating-point number to an integer, with a choice of methods

    Parameters
    ----------
    float : InputFloat
        Float

    Outputs
    -------
    integer : IntegerSocket
        Integer
    """

    _bl_idname = "FunctionNodeFloatToInt"
    node: bpy.types.FunctionNodeFloatToInt

    class Inputs(SocketAccessor):
        float: Socket

    class Outputs(SocketAccessor):
        integer: IntegerSocket

    @property
    def i(self) -> "FloatToInteger.Inputs":
        return FloatToInteger.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "FloatToInteger.Outputs":
        return FloatToInteger.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        float: InputFloat = 0.0,
        *,
        rounding_mode: Literal["ROUND", "FLOOR", "CEILING", "TRUNCATE"] = "ROUND",
    ):
        super().__init__()
        key_args = {"Float": float}
        self.rounding_mode = rounding_mode
        self._establish_links(**key_args)

    @property
    def rounding_mode(self) -> Literal["ROUND", "FLOOR", "CEILING", "TRUNCATE"]:
        return self.node.rounding_mode

    @rounding_mode.setter
    def rounding_mode(self, value: Literal["ROUND", "FLOOR", "CEILING", "TRUNCATE"]):
        self.node.rounding_mode = value


class HashValue(NodeBuilder):
    """
    Generate a randomized integer using the given input value as a seed

    Parameters
    ----------
    value : InputInteger
        Value
    seed : InputInteger
        Seed

    Outputs
    -------
    hash : IntegerSocket
        Hash
    """

    _bl_idname = "FunctionNodeHashValue"
    node: bpy.types.FunctionNodeHashValue

    class Inputs(SocketAccessor):
        value: Socket
        seed: Socket

    class Outputs(SocketAccessor):
        hash: IntegerSocket

    @property
    def i(self) -> "HashValue.Inputs":
        return HashValue.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "HashValue.Outputs":
        return HashValue.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        value: InputInteger = 0,
        seed: InputInteger = 0,
        *,
        data_type: Literal[
            "FLOAT", "INT", "VECTOR", "RGBA", "ROTATION", "MATRIX", "STRING"
        ] = "INT",
    ):
        super().__init__()
        key_args = {"Value": value, "Seed": seed}
        self.data_type = data_type
        self._establish_links(**key_args)

    @classmethod
    def float(cls, value: InputFloat = 0.0, seed: InputInteger = 0) -> "HashValue":
        """Create Hash Value with operation 'Float'."""
        return cls(data_type="FLOAT", value=value, seed=seed)

    @classmethod
    def integer(cls, value: InputInteger = 0, seed: InputInteger = 0) -> "HashValue":
        """Create Hash Value with operation 'Integer'."""
        return cls(data_type="INT", value=value, seed=seed)

    @classmethod
    def vector(cls, value: InputVector = None, seed: InputInteger = 0) -> "HashValue":
        """Create Hash Value with operation 'Vector'."""
        return cls(data_type="VECTOR", value=value, seed=seed)

    @classmethod
    def color(cls, value: InputColor = None, seed: InputInteger = 0) -> "HashValue":
        """Create Hash Value with operation 'Color'."""
        return cls(data_type="RGBA", value=value, seed=seed)

    @classmethod
    def rotation(
        cls, value: InputRotation = None, seed: InputInteger = 0
    ) -> "HashValue":
        """Create Hash Value with operation 'Rotation'."""
        return cls(data_type="ROTATION", value=value, seed=seed)

    @classmethod
    def matrix(cls, value: InputMatrix = None, seed: InputInteger = 0) -> "HashValue":
        """Create Hash Value with operation 'Matrix'."""
        return cls(data_type="MATRIX", value=value, seed=seed)

    @classmethod
    def string(cls, value: InputString = "", seed: InputInteger = 0) -> "HashValue":
        """Create Hash Value with operation 'String'."""
        return cls(data_type="STRING", value=value, seed=seed)

    @property
    def data_type(
        self,
    ) -> Literal["FLOAT", "INT", "VECTOR", "RGBA", "ROTATION", "MATRIX", "STRING"]:
        return self.node.data_type

    @data_type.setter
    def data_type(
        self,
        value: Literal[
            "FLOAT", "INT", "VECTOR", "RGBA", "ROTATION", "MATRIX", "STRING"
        ],
    ):
        self.node.data_type = value


class IndexOfNearest(NodeBuilder):
    """
    Find the nearest element in a group. Similar to the "Sample Nearest" node

    Parameters
    ----------
    position : InputVector
        Position
    group_id : InputInteger
        Group ID

    Outputs
    -------
    index : IntegerSocket
        Index
    has_neighbor : BooleanSocket
        Has Neighbor
    """

    _bl_idname = "GeometryNodeIndexOfNearest"
    node: bpy.types.GeometryNodeIndexOfNearest

    class Inputs(SocketAccessor):
        position: Socket
        group_id: Socket

    class Outputs(SocketAccessor):
        index: IntegerSocket
        has_neighbor: BooleanSocket

    @property
    def i(self) -> "IndexOfNearest.Inputs":
        return IndexOfNearest.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "IndexOfNearest.Outputs":
        return IndexOfNearest.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        position: InputVector = None,
        group_id: InputInteger = 0,
    ):
        super().__init__()
        key_args = {"Position": position, "Group ID": group_id}

        self._establish_links(**key_args)


class IntegerMath(NodeBuilder):
    """
    Perform various math operations on the given integer inputs

    Parameters
    ----------
    value : InputInteger
        Value
    value_001 : InputInteger
        Value
    value_002 : InputInteger
        Value

    Outputs
    -------
    value : IntegerSocket
        Value
    """

    _bl_idname = "FunctionNodeIntegerMath"
    node: bpy.types.FunctionNodeIntegerMath

    class Inputs(SocketAccessor):
        value: Socket
        value_001: Socket
        value_002: Socket

    class Outputs(SocketAccessor):
        value: IntegerSocket

    @property
    def i(self) -> "IntegerMath.Inputs":
        return IntegerMath.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "IntegerMath.Outputs":
        return IntegerMath.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        value: InputInteger = 0,
        value_001: InputInteger = 0,
        value_002: InputInteger = 0,
        *,
        operation: Literal[
            "ADD",
            "SUBTRACT",
            "MULTIPLY",
            "DIVIDE",
            "MULTIPLY_ADD",
            "ABSOLUTE",
            "NEGATE",
            "POWER",
            "MINIMUM",
            "MAXIMUM",
            "SIGN",
            "DIVIDE_ROUND",
            "DIVIDE_FLOOR",
            "DIVIDE_CEIL",
            "FLOORED_MODULO",
            "MODULO",
            "GCD",
            "LCM",
        ] = "ADD",
    ):
        super().__init__()
        key_args = {"Value": value, "Value_001": value_001, "Value_002": value_002}
        self.operation = operation
        self._establish_links(**key_args)

    @classmethod
    def add(cls, value: InputInteger = 0, value_001: InputInteger = 0) -> "IntegerMath":
        """Create Integer Math with operation 'Add'. A + B"""
        return cls(operation="ADD", value=value, value_001=value_001)

    @classmethod
    def subtract(
        cls, value: InputInteger = 0, value_001: InputInteger = 0
    ) -> "IntegerMath":
        """Create Integer Math with operation 'Subtract'. A - B"""
        return cls(operation="SUBTRACT", value=value, value_001=value_001)

    @classmethod
    def multiply(
        cls, value: InputInteger = 0, value_001: InputInteger = 0
    ) -> "IntegerMath":
        """Create Integer Math with operation 'Multiply'. A * B"""
        return cls(operation="MULTIPLY", value=value, value_001=value_001)

    @classmethod
    def divide(
        cls, value: InputInteger = 0, value_001: InputInteger = 0
    ) -> "IntegerMath":
        """Create Integer Math with operation 'Divide'. A / B"""
        return cls(operation="DIVIDE", value=value, value_001=value_001)

    @classmethod
    def multiply_add(
        cls,
        value: InputInteger = 0,
        value_001: InputInteger = 0,
        value_002: InputInteger = 0,
    ) -> "IntegerMath":
        """Create Integer Math with operation 'Multiply Add'. A * B + C"""
        return cls(
            operation="MULTIPLY_ADD",
            value=value,
            value_001=value_001,
            value_002=value_002,
        )

    @classmethod
    def absolute(cls, value: InputInteger = 0) -> "IntegerMath":
        """Create Integer Math with operation 'Absolute'. Non-negative value of A, abs(A)"""
        return cls(operation="ABSOLUTE", value=value)

    @classmethod
    def negate(cls, value: InputInteger = 0) -> "IntegerMath":
        """Create Integer Math with operation 'Negate'. -A"""
        return cls(operation="NEGATE", value=value)

    @classmethod
    def power(
        cls, value: InputInteger = 0, value_001: InputInteger = 0
    ) -> "IntegerMath":
        """Create Integer Math with operation 'Power'. A power B, pow(A,B)"""
        return cls(operation="POWER", value=value, value_001=value_001)

    @classmethod
    def minimum(
        cls, value: InputInteger = 0, value_001: InputInteger = 0
    ) -> "IntegerMath":
        """Create Integer Math with operation 'Minimum'. The minimum value from A and B, min(A,B)"""
        return cls(operation="MINIMUM", value=value, value_001=value_001)

    @classmethod
    def maximum(
        cls, value: InputInteger = 0, value_001: InputInteger = 0
    ) -> "IntegerMath":
        """Create Integer Math with operation 'Maximum'. The maximum value from A and B, max(A,B)"""
        return cls(operation="MAXIMUM", value=value, value_001=value_001)

    @classmethod
    def sign(cls, value: InputInteger = 0) -> "IntegerMath":
        """Create Integer Math with operation 'Sign'. Return the sign of A, sign(A)"""
        return cls(operation="SIGN", value=value)

    @classmethod
    def divide_round(
        cls, value: InputInteger = 0, value_001: InputInteger = 0
    ) -> "IntegerMath":
        """Create Integer Math with operation 'Divide Round'. Divide and round result toward zero"""
        return cls(operation="DIVIDE_ROUND", value=value, value_001=value_001)

    @classmethod
    def divide_floor(
        cls, value: InputInteger = 0, value_001: InputInteger = 0
    ) -> "IntegerMath":
        """Create Integer Math with operation 'Divide Floor'. Divide and floor result, the largest integer smaller than or equal A"""
        return cls(operation="DIVIDE_FLOOR", value=value, value_001=value_001)

    @classmethod
    def divide_ceiling(
        cls, value: InputInteger = 0, value_001: InputInteger = 0
    ) -> "IntegerMath":
        """Create Integer Math with operation 'Divide Ceiling'. Divide and ceil result, the smallest integer greater than or equal A"""
        return cls(operation="DIVIDE_CEIL", value=value, value_001=value_001)

    @classmethod
    def floored_modulo(
        cls, value: InputInteger = 0, value_001: InputInteger = 0
    ) -> "IntegerMath":
        """Create Integer Math with operation 'Floored Modulo'. Modulo that is periodic for both negative and positive operands"""
        return cls(operation="FLOORED_MODULO", value=value, value_001=value_001)

    @classmethod
    def modulo(
        cls, value: InputInteger = 0, value_001: InputInteger = 0
    ) -> "IntegerMath":
        """Create Integer Math with operation 'Modulo'. Modulo which is the remainder of A / B"""
        return cls(operation="MODULO", value=value, value_001=value_001)

    @classmethod
    def greatest_common_divisor(
        cls, value: InputInteger = 0, value_001: InputInteger = 0
    ) -> "IntegerMath":
        """Create Integer Math with operation 'Greatest Common Divisor'. The largest positive integer that divides into each of the values A and B, e.g. GCD(8,12) = 4"""
        return cls(operation="GCD", value=value, value_001=value_001)

    @classmethod
    def least_common_multiple(
        cls, value: InputInteger = 0, value_001: InputInteger = 0
    ) -> "IntegerMath":
        """Create Integer Math with operation 'Least Common Multiple'. The smallest positive integer that is divisible by both A and B, e.g. LCM(6,10) = 30"""
        return cls(operation="LCM", value=value, value_001=value_001)

    @property
    def operation(
        self,
    ) -> Literal[
        "ADD",
        "SUBTRACT",
        "MULTIPLY",
        "DIVIDE",
        "MULTIPLY_ADD",
        "ABSOLUTE",
        "NEGATE",
        "POWER",
        "MINIMUM",
        "MAXIMUM",
        "SIGN",
        "DIVIDE_ROUND",
        "DIVIDE_FLOOR",
        "DIVIDE_CEIL",
        "FLOORED_MODULO",
        "MODULO",
        "GCD",
        "LCM",
    ]:
        return self.node.operation

    @operation.setter
    def operation(
        self,
        value: Literal[
            "ADD",
            "SUBTRACT",
            "MULTIPLY",
            "DIVIDE",
            "MULTIPLY_ADD",
            "ABSOLUTE",
            "NEGATE",
            "POWER",
            "MINIMUM",
            "MAXIMUM",
            "SIGN",
            "DIVIDE_ROUND",
            "DIVIDE_FLOOR",
            "DIVIDE_CEIL",
            "FLOORED_MODULO",
            "MODULO",
            "GCD",
            "LCM",
        ],
    ):
        self.node.operation = value


class InvertMatrix(NodeBuilder):
    """
    Compute the inverse of the given matrix, if one exists

    Parameters
    ----------
    matrix : InputMatrix
        Matrix

    Outputs
    -------
    matrix : MatrixSocket
        Matrix
    invertible : BooleanSocket
        Invertible
    """

    _bl_idname = "FunctionNodeInvertMatrix"
    node: bpy.types.FunctionNodeInvertMatrix

    class Inputs(SocketAccessor):
        matrix: Socket

    class Outputs(SocketAccessor):
        matrix: MatrixSocket
        invertible: BooleanSocket

    @property
    def i(self) -> "InvertMatrix.Inputs":
        return InvertMatrix.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "InvertMatrix.Outputs":
        return InvertMatrix.Outputs(self.node.outputs, "output")

    def __init__(self, matrix: InputMatrix = None):
        super().__init__()
        key_args = {"Matrix": matrix}

        self._establish_links(**key_args)


class InvertRotation(NodeBuilder):
    """
    Compute the inverse of the given rotation

    Parameters
    ----------
    rotation : InputRotation
        Rotation

    Outputs
    -------
    rotation : RotationSocket
        Rotation
    """

    _bl_idname = "FunctionNodeInvertRotation"
    node: bpy.types.FunctionNodeInvertRotation

    class Inputs(SocketAccessor):
        rotation: Socket

    class Outputs(SocketAccessor):
        rotation: RotationSocket

    @property
    def i(self) -> "InvertRotation.Inputs":
        return InvertRotation.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "InvertRotation.Outputs":
        return InvertRotation.Outputs(self.node.outputs, "output")

    def __init__(self, rotation: InputRotation = None):
        super().__init__()
        key_args = {"Rotation": rotation}

        self._establish_links(**key_args)


class JoinBundle(NodeBuilder):
    """
    Join multiple bundles together

    Parameters
    ----------
    bundle : InputBundle
        Bundle

    Outputs
    -------
    bundle : BundleSocket
        Bundle
    """

    _bl_idname = "NodeJoinBundle"
    node: bpy.types.Node

    class Inputs(SocketAccessor):
        bundle: Socket

    class Outputs(SocketAccessor):
        bundle: BundleSocket

    @property
    def i(self) -> "JoinBundle.Inputs":
        return JoinBundle.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "JoinBundle.Outputs":
        return JoinBundle.Outputs(self.node.outputs, "output")

    def __init__(self, bundle: InputBundle = None):
        super().__init__()
        key_args = {"Bundle": bundle}

        self._establish_links(**key_args)


class MapRange(NodeBuilder):
    """
    Remap a value from a range to a target range

    Parameters
    ----------
    value : InputFloat
        Value
    from_min : InputFloat
        From Min
    from_max : InputFloat
        From Max
    to_min : InputFloat
        To Min
    to_max : InputFloat
        To Max
    steps : InputFloat
        Steps
    vector : InputVector
        Vector
    from_min_float3 : InputVector
        From Min
    from_max_float3 : InputVector
        From Max
    to_min_float3 : InputVector
        To Min
    to_max_float3 : InputVector
        To Max
    steps_float3 : InputVector
        Steps

    Outputs
    -------
    result : FloatSocket
        Result
    vector : VectorSocket
        Vector
    """

    _bl_idname = "ShaderNodeMapRange"
    node: bpy.types.ShaderNodeMapRange

    class Inputs(SocketAccessor):
        value: Socket
        from_min: Socket
        from_max: Socket
        to_min: Socket
        to_max: Socket
        steps: Socket
        vector: Socket
        from_min_float3: Socket
        from_max_float3: Socket
        to_min_float3: Socket
        to_max_float3: Socket
        steps_float3: Socket

    class Outputs(SocketAccessor):
        result: FloatSocket
        vector: VectorSocket

    @property
    def i(self) -> "MapRange.Inputs":
        return MapRange.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "MapRange.Outputs":
        return MapRange.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        value: InputFloat = 1.0,
        from_min: InputFloat = 0.0,
        from_max: InputFloat = 1.0,
        to_min: InputFloat = 0.0,
        to_max: InputFloat = 1.0,
        steps: InputFloat = 4.0,
        vector: InputVector = None,
        from_min_float3: InputVector = None,
        from_max_float3: InputVector = None,
        to_min_float3: InputVector = None,
        to_max_float3: InputVector = None,
        steps_float3: InputVector = None,
        *,
        clamp: bool = False,
        interpolation_type: Literal[
            "LINEAR", "STEPPED", "SMOOTHSTEP", "SMOOTHERSTEP"
        ] = "LINEAR",
        data_type: Literal["FLOAT", "FLOAT_VECTOR"] = "FLOAT",
    ):
        super().__init__()
        key_args = {
            "Value": value,
            "From Min": from_min,
            "From Max": from_max,
            "To Min": to_min,
            "To Max": to_max,
            "Steps": steps,
            "Vector": vector,
            "From_Min_FLOAT3": from_min_float3,
            "From_Max_FLOAT3": from_max_float3,
            "To_Min_FLOAT3": to_min_float3,
            "To_Max_FLOAT3": to_max_float3,
            "Steps_FLOAT3": steps_float3,
        }
        self.clamp = clamp
        self.interpolation_type = interpolation_type
        self.data_type = data_type
        self._establish_links(**key_args)

    @classmethod
    def linear(
        cls,
        value: InputFloat = 1.0,
        from_min: InputFloat = 0.0,
        from_max: InputFloat = 1.0,
        to_min: InputFloat = 0.0,
        to_max: InputFloat = 1.0,
    ) -> "MapRange":
        """Create Map Range with operation 'Linear'. Linear interpolation between From Min and From Max values"""
        return cls(
            interpolation_type="LINEAR",
            value=value,
            from_min=from_min,
            from_max=from_max,
            to_min=to_min,
            to_max=to_max,
        )

    @classmethod
    def stepped_linear(
        cls,
        value: InputFloat = 1.0,
        from_min: InputFloat = 0.0,
        from_max: InputFloat = 1.0,
        to_min: InputFloat = 0.0,
        to_max: InputFloat = 1.0,
        steps: InputFloat = 4.0,
    ) -> "MapRange":
        """Create Map Range with operation 'Stepped Linear'. Stepped linear interpolation between From Min and From Max values"""
        return cls(
            interpolation_type="STEPPED",
            value=value,
            from_min=from_min,
            from_max=from_max,
            to_min=to_min,
            to_max=to_max,
            steps=steps,
        )

    @classmethod
    def smooth_step(
        cls,
        value: InputFloat = 1.0,
        from_min: InputFloat = 0.0,
        from_max: InputFloat = 1.0,
        to_min: InputFloat = 0.0,
        to_max: InputFloat = 1.0,
    ) -> "MapRange":
        """Create Map Range with operation 'Smooth Step'. Smooth Hermite edge interpolation between From Min and From Max values"""
        return cls(
            interpolation_type="SMOOTHSTEP",
            value=value,
            from_min=from_min,
            from_max=from_max,
            to_min=to_min,
            to_max=to_max,
        )

    @classmethod
    def smoother_step(
        cls,
        value: InputFloat = 1.0,
        from_min: InputFloat = 0.0,
        from_max: InputFloat = 1.0,
        to_min: InputFloat = 0.0,
        to_max: InputFloat = 1.0,
    ) -> "MapRange":
        """Create Map Range with operation 'Smoother Step'. Smoother Hermite edge interpolation between From Min and From Max values"""
        return cls(
            interpolation_type="SMOOTHERSTEP",
            value=value,
            from_min=from_min,
            from_max=from_max,
            to_min=to_min,
            to_max=to_max,
        )

    @classmethod
    def float(
        cls,
        value: InputFloat = 1.0,
        from_min: InputFloat = 0.0,
        from_max: InputFloat = 1.0,
        to_min: InputFloat = 0.0,
        to_max: InputFloat = 1.0,
    ) -> "MapRange":
        """Create Map Range with operation 'Float'. Floating-point value"""
        return cls(
            data_type="FLOAT",
            value=value,
            from_min=from_min,
            from_max=from_max,
            to_min=to_min,
            to_max=to_max,
        )

    @classmethod
    def vector(
        cls,
        vector: InputVector = None,
        from_min3: InputVector = None,
        from_max3: InputVector = None,
        to_min3: InputVector = None,
        to_max3: InputVector = None,
    ) -> "MapRange":
        """Create Map Range with operation 'Vector'. 3D vector with floating-point values"""
        return cls(
            data_type="FLOAT_VECTOR",
            vector=vector,
            from_min_float3=from_min3,
            from_max_float3=from_max3,
            to_min_float3=to_min3,
            to_max_float3=to_max3,
        )

    @property
    def clamp(self) -> bool:
        return self.node.clamp

    @clamp.setter
    def clamp(self, value: bool):
        self.node.clamp = value

    @property
    def interpolation_type(
        self,
    ) -> Literal["LINEAR", "STEPPED", "SMOOTHSTEP", "SMOOTHERSTEP"]:
        return self.node.interpolation_type

    @interpolation_type.setter
    def interpolation_type(
        self, value: Literal["LINEAR", "STEPPED", "SMOOTHSTEP", "SMOOTHERSTEP"]
    ):
        self.node.interpolation_type = value

    @property
    def data_type(self) -> Literal["FLOAT", "FLOAT_VECTOR"]:
        return self.node.data_type

    @data_type.setter
    def data_type(self, value: Literal["FLOAT", "FLOAT_VECTOR"]):
        self.node.data_type = value


class MatchString(NodeBuilder):
    """
    Check if a given string exists within another string

    Parameters
    ----------
    string : InputString
        String
    operation : InputMenu | Literal['Starts With', 'Ends With', 'Contains']
        Operation
    key : InputString
        Key

    Outputs
    -------
    result : BooleanSocket
        Result
    """

    _bl_idname = "FunctionNodeMatchString"
    node: bpy.types.FunctionNodeMatchString

    class Inputs(SocketAccessor):
        string: Socket
        operation: Socket
        key: Socket

    class Outputs(SocketAccessor):
        result: BooleanSocket

    @property
    def i(self) -> "MatchString.Inputs":
        return MatchString.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "MatchString.Outputs":
        return MatchString.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        string: InputString = "",
        operation: InputMenu
        | Literal["Starts With", "Ends With", "Contains"] = "Starts With",
        key: InputString = "",
    ):
        super().__init__()
        key_args = {"String": string, "Operation": operation, "Key": key}

        self._establish_links(**key_args)


class Math(NodeBuilder):
    """
    Perform math operations

    Parameters
    ----------
    value : InputFloat
        Value
    value_001 : InputFloat
        Value
    value_002 : InputFloat
        Value

    Outputs
    -------
    value : FloatSocket
        Value
    """

    _bl_idname = "ShaderNodeMath"
    node: bpy.types.ShaderNodeMath

    class Inputs(SocketAccessor):
        value: Socket
        value_001: Socket
        value_002: Socket

    class Outputs(SocketAccessor):
        value: FloatSocket

    @property
    def i(self) -> "Math.Inputs":
        return Math.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "Math.Outputs":
        return Math.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        value: InputFloat = 0.5,
        value_001: InputFloat = 0.5,
        value_002: InputFloat = 0.5,
        *,
        operation: Literal[
            "ADD",
            "SUBTRACT",
            "MULTIPLY",
            "DIVIDE",
            "MULTIPLY_ADD",
            "POWER",
            "LOGARITHM",
            "SQRT",
            "INVERSE_SQRT",
            "ABSOLUTE",
            "EXPONENT",
            "MINIMUM",
            "MAXIMUM",
            "LESS_THAN",
            "GREATER_THAN",
            "SIGN",
            "COMPARE",
            "SMOOTH_MIN",
            "SMOOTH_MAX",
            "ROUND",
            "FLOOR",
            "CEIL",
            "TRUNC",
            "FRACT",
            "MODULO",
            "FLOORED_MODULO",
            "WRAP",
            "SNAP",
            "PINGPONG",
            "SINE",
            "COSINE",
            "TANGENT",
            "ARCSINE",
            "ARCCOSINE",
            "ARCTANGENT",
            "ARCTAN2",
            "SINH",
            "COSH",
            "TANH",
            "RADIANS",
            "DEGREES",
        ] = "ADD",
        use_clamp: bool = False,
    ):
        super().__init__()
        key_args = {"Value": value, "Value_001": value_001, "Value_002": value_002}
        self.operation = operation
        self.use_clamp = use_clamp
        self._establish_links(**key_args)

    @classmethod
    def add(cls, value: InputFloat = 0.5, value_001: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'Add'. A + B"""
        return cls(operation="ADD", value=value, value_001=value_001)

    @classmethod
    def subtract(cls, value: InputFloat = 0.5, value_001: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'Subtract'. A - B"""
        return cls(operation="SUBTRACT", value=value, value_001=value_001)

    @classmethod
    def multiply(cls, value: InputFloat = 0.5, value_001: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'Multiply'. A * B"""
        return cls(operation="MULTIPLY", value=value, value_001=value_001)

    @classmethod
    def divide(cls, value: InputFloat = 0.5, value_001: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'Divide'. A / B"""
        return cls(operation="DIVIDE", value=value, value_001=value_001)

    @classmethod
    def multiply_add(
        cls,
        value: InputFloat = 0.5,
        value_001: InputFloat = 0.5,
        value_002: InputFloat = 0.5,
    ) -> "Math":
        """Create Math with operation 'Multiply Add'. A * B + C"""
        return cls(
            operation="MULTIPLY_ADD",
            value=value,
            value_001=value_001,
            value_002=value_002,
        )

    @classmethod
    def power(cls, value: InputFloat = 0.5, value_001: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'Power'. A power B"""
        return cls(operation="POWER", value=value, value_001=value_001)

    @classmethod
    def logarithm(cls, value: InputFloat = 0.5, value_001: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'Logarithm'. Logarithm A base B"""
        return cls(operation="LOGARITHM", value=value, value_001=value_001)

    @classmethod
    def square_root(cls, value: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'Square Root'. Square root of A"""
        return cls(operation="SQRT", value=value)

    @classmethod
    def inverse_square_root(cls, value: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'Inverse Square Root'. 1 / Square root of A"""
        return cls(operation="INVERSE_SQRT", value=value)

    @classmethod
    def absolute(cls, value: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'Absolute'. Magnitude of A"""
        return cls(operation="ABSOLUTE", value=value)

    @classmethod
    def exponent(cls, value: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'Exponent'. exp(A)"""
        return cls(operation="EXPONENT", value=value)

    @classmethod
    def minimum(cls, value: InputFloat = 0.5, value_001: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'Minimum'. The minimum from A and B"""
        return cls(operation="MINIMUM", value=value, value_001=value_001)

    @classmethod
    def maximum(cls, value: InputFloat = 0.5, value_001: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'Maximum'. The maximum from A and B"""
        return cls(operation="MAXIMUM", value=value, value_001=value_001)

    @classmethod
    def less_than(cls, value: InputFloat = 0.5, value_001: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'Less Than'. 1 if A < B else 0"""
        return cls(operation="LESS_THAN", value=value, value_001=value_001)

    @classmethod
    def greater_than(
        cls, value: InputFloat = 0.5, value_001: InputFloat = 0.5
    ) -> "Math":
        """Create Math with operation 'Greater Than'. 1 if A > B else 0"""
        return cls(operation="GREATER_THAN", value=value, value_001=value_001)

    @classmethod
    def sign(cls, value: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'Sign'. Returns the sign of A"""
        return cls(operation="SIGN", value=value)

    @classmethod
    def compare(
        cls,
        value: InputFloat = 0.5,
        value_001: InputFloat = 0.5,
        value_002: InputFloat = 0.5,
    ) -> "Math":
        """Create Math with operation 'Compare'. 1 if (A == B) within tolerance C else 0"""
        return cls(
            operation="COMPARE", value=value, value_001=value_001, value_002=value_002
        )

    @classmethod
    def smooth_minimum(
        cls,
        value: InputFloat = 0.5,
        value_001: InputFloat = 0.5,
        value_002: InputFloat = 0.5,
    ) -> "Math":
        """Create Math with operation 'Smooth Minimum'. The minimum from A and B with smoothing C"""
        return cls(
            operation="SMOOTH_MIN",
            value=value,
            value_001=value_001,
            value_002=value_002,
        )

    @classmethod
    def smooth_maximum(
        cls,
        value: InputFloat = 0.5,
        value_001: InputFloat = 0.5,
        value_002: InputFloat = 0.5,
    ) -> "Math":
        """Create Math with operation 'Smooth Maximum'. The maximum from A and B with smoothing C"""
        return cls(
            operation="SMOOTH_MAX",
            value=value,
            value_001=value_001,
            value_002=value_002,
        )

    @classmethod
    def round(cls, value: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'Round'. Round A to the nearest integer. Round upward if the fraction part is 0.5"""
        return cls(operation="ROUND", value=value)

    @classmethod
    def floor(cls, value: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'Floor'. The largest integer smaller than or equal A"""
        return cls(operation="FLOOR", value=value)

    @classmethod
    def ceil(cls, value: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'Ceil'. The smallest integer greater than or equal A"""
        return cls(operation="CEIL", value=value)

    @classmethod
    def truncate(cls, value: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'Truncate'. The integer part of A, removing fractional digits"""
        return cls(operation="TRUNC", value=value)

    @classmethod
    def fraction(cls, value: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'Fraction'. The fraction part of A"""
        return cls(operation="FRACT", value=value)

    @classmethod
    def truncated_modulo(
        cls, value: InputFloat = 0.5, value_001: InputFloat = 0.5
    ) -> "Math":
        """Create Math with operation 'Truncated Modulo'. The remainder of truncated division using fmod(A,B)"""
        return cls(operation="MODULO", value=value, value_001=value_001)

    @classmethod
    def floored_modulo(
        cls, value: InputFloat = 0.5, value_001: InputFloat = 0.5
    ) -> "Math":
        """Create Math with operation 'Floored Modulo'. The remainder of floored division"""
        return cls(operation="FLOORED_MODULO", value=value, value_001=value_001)

    @classmethod
    def wrap(
        cls,
        value: InputFloat = 0.5,
        value_001: InputFloat = 0.5,
        value_002: InputFloat = 0.5,
    ) -> "Math":
        """Create Math with operation 'Wrap'. Wrap value to range, wrap(A,B)"""
        return cls(
            operation="WRAP", value=value, value_001=value_001, value_002=value_002
        )

    @classmethod
    def snap(cls, value: InputFloat = 0.5, value_001: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'Snap'. Snap to increment, snap(A,B)"""
        return cls(operation="SNAP", value=value, value_001=value_001)

    @classmethod
    def ping_pong(cls, value: InputFloat = 0.5, value_001: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'Ping-Pong'. Wraps a value and reverses every other cycle (A,B)"""
        return cls(operation="PINGPONG", value=value, value_001=value_001)

    @classmethod
    def sine(cls, value: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'Sine'. sin(A)"""
        return cls(operation="SINE", value=value)

    @classmethod
    def cosine(cls, value: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'Cosine'. cos(A)"""
        return cls(operation="COSINE", value=value)

    @classmethod
    def tangent(cls, value: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'Tangent'. tan(A)"""
        return cls(operation="TANGENT", value=value)

    @classmethod
    def arcsine(cls, value: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'Arcsine'. arcsin(A)"""
        return cls(operation="ARCSINE", value=value)

    @classmethod
    def arccosine(cls, value: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'Arccosine'. arccos(A)"""
        return cls(operation="ARCCOSINE", value=value)

    @classmethod
    def arctangent(cls, value: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'Arctangent'. arctan(A)"""
        return cls(operation="ARCTANGENT", value=value)

    @classmethod
    def arctan2(cls, value: InputFloat = 0.5, value_001: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'Arctan2'. The signed angle arctan(A / B)"""
        return cls(operation="ARCTAN2", value=value, value_001=value_001)

    @classmethod
    def hyperbolic_sine(cls, value: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'Hyperbolic Sine'. sinh(A)"""
        return cls(operation="SINH", value=value)

    @classmethod
    def hyperbolic_cosine(cls, value: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'Hyperbolic Cosine'. cosh(A)"""
        return cls(operation="COSH", value=value)

    @classmethod
    def hyperbolic_tangent(cls, value: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'Hyperbolic Tangent'. tanh(A)"""
        return cls(operation="TANH", value=value)

    @classmethod
    def to_radians(cls, value: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'To Radians'. Convert from degrees to radians"""
        return cls(operation="RADIANS", value=value)

    @classmethod
    def to_degrees(cls, value: InputFloat = 0.5) -> "Math":
        """Create Math with operation 'To Degrees'. Convert from radians to degrees"""
        return cls(operation="DEGREES", value=value)

    @property
    def operation(
        self,
    ) -> Literal[
        "ADD",
        "SUBTRACT",
        "MULTIPLY",
        "DIVIDE",
        "MULTIPLY_ADD",
        "POWER",
        "LOGARITHM",
        "SQRT",
        "INVERSE_SQRT",
        "ABSOLUTE",
        "EXPONENT",
        "MINIMUM",
        "MAXIMUM",
        "LESS_THAN",
        "GREATER_THAN",
        "SIGN",
        "COMPARE",
        "SMOOTH_MIN",
        "SMOOTH_MAX",
        "ROUND",
        "FLOOR",
        "CEIL",
        "TRUNC",
        "FRACT",
        "MODULO",
        "FLOORED_MODULO",
        "WRAP",
        "SNAP",
        "PINGPONG",
        "SINE",
        "COSINE",
        "TANGENT",
        "ARCSINE",
        "ARCCOSINE",
        "ARCTANGENT",
        "ARCTAN2",
        "SINH",
        "COSH",
        "TANH",
        "RADIANS",
        "DEGREES",
    ]:
        return self.node.operation

    @operation.setter
    def operation(
        self,
        value: Literal[
            "ADD",
            "SUBTRACT",
            "MULTIPLY",
            "DIVIDE",
            "MULTIPLY_ADD",
            "POWER",
            "LOGARITHM",
            "SQRT",
            "INVERSE_SQRT",
            "ABSOLUTE",
            "EXPONENT",
            "MINIMUM",
            "MAXIMUM",
            "LESS_THAN",
            "GREATER_THAN",
            "SIGN",
            "COMPARE",
            "SMOOTH_MIN",
            "SMOOTH_MAX",
            "ROUND",
            "FLOOR",
            "CEIL",
            "TRUNC",
            "FRACT",
            "MODULO",
            "FLOORED_MODULO",
            "WRAP",
            "SNAP",
            "PINGPONG",
            "SINE",
            "COSINE",
            "TANGENT",
            "ARCSINE",
            "ARCCOSINE",
            "ARCTANGENT",
            "ARCTAN2",
            "SINH",
            "COSH",
            "TANH",
            "RADIANS",
            "DEGREES",
        ],
    ):
        self.node.operation = value

    @property
    def use_clamp(self) -> bool:
        return self.node.use_clamp

    @use_clamp.setter
    def use_clamp(self, value: bool):
        self.node.use_clamp = value


class MatrixDeterminant(NodeBuilder):
    """
    Compute the determinant of the given matrix

    Parameters
    ----------
    matrix : InputMatrix
        Matrix

    Outputs
    -------
    determinant : FloatSocket
        Determinant
    """

    _bl_idname = "FunctionNodeMatrixDeterminant"
    node: bpy.types.FunctionNodeMatrixDeterminant

    class Inputs(SocketAccessor):
        matrix: Socket

    class Outputs(SocketAccessor):
        determinant: FloatSocket

    @property
    def i(self) -> "MatrixDeterminant.Inputs":
        return MatrixDeterminant.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "MatrixDeterminant.Outputs":
        return MatrixDeterminant.Outputs(self.node.outputs, "output")

    def __init__(self, matrix: InputMatrix = None):
        super().__init__()
        key_args = {"Matrix": matrix}

        self._establish_links(**key_args)


class Mix(NodeBuilder):
    """
    Mix values by a factor

    Parameters
    ----------
    factor_float : InputFloat
        Factor
    factor_vector : InputVector
        Factor
    a_float : InputFloat
        A
    b_float : InputFloat
        B
    a_vector : InputVector
        A
    b_vector : InputVector
        B
    a_color : InputColor
        A
    b_color : InputColor
        B
    a_rotation : InputRotation
        A
    b_rotation : InputRotation
        B

    Outputs
    -------
    result_float : FloatSocket
        Result
    result_vector : VectorSocket
        Result
    result_color : ColorSocket
        Result
    result_rotation : RotationSocket
        Result
    """

    _bl_idname = "ShaderNodeMix"
    node: bpy.types.ShaderNodeMix

    class Inputs(SocketAccessor):
        factor_float: Socket
        factor_vector: Socket
        a_float: Socket
        b_float: Socket
        a_vector: Socket
        b_vector: Socket
        a_color: Socket
        b_color: Socket
        a_rotation: Socket
        b_rotation: Socket

    class Outputs(SocketAccessor):
        result_float: FloatSocket
        result_vector: VectorSocket
        result_color: ColorSocket
        result_rotation: RotationSocket

    @property
    def i(self) -> "Mix.Inputs":
        return Mix.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "Mix.Outputs":
        return Mix.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        factor_float: InputFloat = 0.5,
        factor_vector: InputVector = None,
        a_float: InputFloat = 0.0,
        b_float: InputFloat = 0.0,
        a_vector: InputVector = None,
        b_vector: InputVector = None,
        a_color: InputColor = None,
        b_color: InputColor = None,
        a_rotation: InputRotation = None,
        b_rotation: InputRotation = None,
        *,
        data_type: Literal["FLOAT", "VECTOR", "RGBA", "ROTATION"] = "FLOAT",
        factor_mode: Literal["UNIFORM", "NON_UNIFORM"] = "UNIFORM",
        blend_type: Literal[
            "MIX",
            "DARKEN",
            "MULTIPLY",
            "BURN",
            "LIGHTEN",
            "SCREEN",
            "DODGE",
            "ADD",
            "OVERLAY",
            "SOFT_LIGHT",
            "LINEAR_LIGHT",
            "DIFFERENCE",
            "EXCLUSION",
            "SUBTRACT",
            "DIVIDE",
            "HUE",
            "SATURATION",
            "COLOR",
            "VALUE",
        ] = "MIX",
        clamp_factor: bool = False,
        clamp_result: bool = False,
    ):
        super().__init__()
        key_args = {
            "Factor_Float": factor_float,
            "Factor_Vector": factor_vector,
            "A_Float": a_float,
            "B_Float": b_float,
            "A_Vector": a_vector,
            "B_Vector": b_vector,
            "A_Color": a_color,
            "B_Color": b_color,
            "A_Rotation": a_rotation,
            "B_Rotation": b_rotation,
        }
        self.data_type = data_type
        self.factor_mode = factor_mode
        self.blend_type = blend_type
        self.clamp_factor = clamp_factor
        self.clamp_result = clamp_result
        self._establish_links(**key_args)

    @classmethod
    def float(
        cls, factor: InputFloat = 0.5, a: InputFloat = 0.0, b: InputFloat = 0.0
    ) -> "Mix":
        """Create Mix with operation 'Float'."""
        return cls(data_type="FLOAT", factor_float=factor, a_float=a, b_float=b)

    @classmethod
    def vector(
        cls, factor: InputFloat = 0.5, a: InputVector = None, b: InputVector = None
    ) -> "Mix":
        """Create Mix with operation 'Vector'."""
        return cls(data_type="VECTOR", factor_float=factor, a_vector=a, b_vector=b)

    @classmethod
    def color(
        cls,
        factor: InputFloat = 0.5,
        a_color: InputColor = None,
        b_color: InputColor = None,
    ) -> "Mix":
        """Create Mix with operation 'Color'."""
        return cls(
            data_type="RGBA", factor_float=factor, a_color=a_color, b_color=b_color
        )

    @classmethod
    def rotation(
        cls,
        factor: InputFloat = 0.5,
        a_rotation: InputRotation = None,
        b_rotation: InputRotation = None,
    ) -> "Mix":
        """Create Mix with operation 'Rotation'."""
        return cls(
            data_type="ROTATION",
            factor_float=factor,
            a_rotation=a_rotation,
            b_rotation=b_rotation,
        )

    @property
    def data_type(self) -> Literal["FLOAT", "VECTOR", "RGBA", "ROTATION"]:
        return self.node.data_type

    @data_type.setter
    def data_type(self, value: Literal["FLOAT", "VECTOR", "RGBA", "ROTATION"]):
        self.node.data_type = value

    @property
    def factor_mode(self) -> Literal["UNIFORM", "NON_UNIFORM"]:
        return self.node.factor_mode

    @factor_mode.setter
    def factor_mode(self, value: Literal["UNIFORM", "NON_UNIFORM"]):
        self.node.factor_mode = value

    @property
    def blend_type(
        self,
    ) -> Literal[
        "MIX",
        "DARKEN",
        "MULTIPLY",
        "BURN",
        "LIGHTEN",
        "SCREEN",
        "DODGE",
        "ADD",
        "OVERLAY",
        "SOFT_LIGHT",
        "LINEAR_LIGHT",
        "DIFFERENCE",
        "EXCLUSION",
        "SUBTRACT",
        "DIVIDE",
        "HUE",
        "SATURATION",
        "COLOR",
        "VALUE",
    ]:
        return self.node.blend_type

    @blend_type.setter
    def blend_type(
        self,
        value: Literal[
            "MIX",
            "DARKEN",
            "MULTIPLY",
            "BURN",
            "LIGHTEN",
            "SCREEN",
            "DODGE",
            "ADD",
            "OVERLAY",
            "SOFT_LIGHT",
            "LINEAR_LIGHT",
            "DIFFERENCE",
            "EXCLUSION",
            "SUBTRACT",
            "DIVIDE",
            "HUE",
            "SATURATION",
            "COLOR",
            "VALUE",
        ],
    ):
        self.node.blend_type = value

    @property
    def clamp_factor(self) -> bool:
        return self.node.clamp_factor

    @clamp_factor.setter
    def clamp_factor(self, value: bool):
        self.node.clamp_factor = value

    @property
    def clamp_result(self) -> bool:
        return self.node.clamp_result

    @clamp_result.setter
    def clamp_result(self, value: bool):
        self.node.clamp_result = value


class MultiplyMatrices(NodeBuilder):
    """
    Perform a matrix multiplication on two input matrices

    Parameters
    ----------
    matrix : InputMatrix
        Matrix
    matrix_001 : InputMatrix
        Matrix

    Outputs
    -------
    matrix : MatrixSocket
        Matrix
    """

    _bl_idname = "FunctionNodeMatrixMultiply"
    node: bpy.types.FunctionNodeMatrixMultiply

    class Inputs(SocketAccessor):
        matrix: Socket
        matrix_001: Socket

    class Outputs(SocketAccessor):
        matrix: MatrixSocket

    @property
    def i(self) -> "MultiplyMatrices.Inputs":
        return MultiplyMatrices.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "MultiplyMatrices.Outputs":
        return MultiplyMatrices.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        matrix: InputMatrix = None,
        matrix_001: InputMatrix = None,
    ):
        super().__init__()
        key_args = {"Matrix": matrix, "Matrix_001": matrix_001}

        self._establish_links(**key_args)


class PackUVIslands(NodeBuilder):
    """
    Scale islands of a UV map and move them so they fill the UV space as much as possible

    Parameters
    ----------
    uv : InputVector
        UV
    selection : InputBoolean
        Selection
    margin : InputFloat
        Margin
    rotate : InputBoolean
        Rotate
    method : InputMenu | Literal['Bounding Box', 'Convex Hull', 'Exact Shape']
        Method

    Outputs
    -------
    uv : VectorSocket
        UV
    """

    _bl_idname = "GeometryNodeUVPackIslands"
    node: bpy.types.GeometryNodeUVPackIslands

    class Inputs(SocketAccessor):
        uv: Socket
        selection: Socket
        margin: Socket
        rotate: Socket
        method: Socket

    class Outputs(SocketAccessor):
        uv: VectorSocket

    @property
    def i(self) -> "PackUVIslands.Inputs":
        return PackUVIslands.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "PackUVIslands.Outputs":
        return PackUVIslands.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        uv: InputVector = None,
        selection: InputBoolean = True,
        margin: InputFloat = 0.001,
        rotate: InputBoolean = True,
        method: InputMenu
        | Literal["Bounding Box", "Convex Hull", "Exact Shape"] = "Bounding Box",
    ):
        super().__init__()
        key_args = {
            "UV": uv,
            "Selection": selection,
            "Margin": margin,
            "Rotate": rotate,
            "Method": method,
        }

        self._establish_links(**key_args)


class ProjectPoint(NodeBuilder):
    """
    Project a point using a matrix, using location, rotation, scale, and perspective divide

    Parameters
    ----------
    vector : InputVector
        Vector
    transform : InputMatrix
        Transform

    Outputs
    -------
    vector : VectorSocket
        Vector
    """

    _bl_idname = "FunctionNodeProjectPoint"
    node: bpy.types.FunctionNodeProjectPoint

    class Inputs(SocketAccessor):
        vector: Socket
        transform: Socket

    class Outputs(SocketAccessor):
        vector: VectorSocket

    @property
    def i(self) -> "ProjectPoint.Inputs":
        return ProjectPoint.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "ProjectPoint.Outputs":
        return ProjectPoint.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        vector: InputVector = None,
        transform: InputMatrix = None,
    ):
        super().__init__()
        key_args = {"Vector": vector, "Transform": transform}

        self._establish_links(**key_args)


class QuaternionToRotation(NodeBuilder):
    """
    Build a rotation from quaternion components

    Parameters
    ----------
    w : InputFloat
        W
    x : InputFloat
        X
    y : InputFloat
        Y
    z : InputFloat
        Z

    Outputs
    -------
    rotation : RotationSocket
        Rotation
    """

    _bl_idname = "FunctionNodeQuaternionToRotation"
    node: bpy.types.FunctionNodeQuaternionToRotation

    class Inputs(SocketAccessor):
        w: Socket
        x: Socket
        y: Socket
        z: Socket

    class Outputs(SocketAccessor):
        rotation: RotationSocket

    @property
    def i(self) -> "QuaternionToRotation.Inputs":
        return QuaternionToRotation.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "QuaternionToRotation.Outputs":
        return QuaternionToRotation.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        w: InputFloat = 1.0,
        x: InputFloat = 0.0,
        y: InputFloat = 0.0,
        z: InputFloat = 0.0,
    ):
        super().__init__()
        key_args = {"W": w, "X": x, "Y": y, "Z": z}

        self._establish_links(**key_args)


class RandomValue(NodeBuilder):
    """
    Output a randomized value

    Parameters
    ----------
    min : InputVector
        Min
    max : InputVector
        Max
    min_001 : InputFloat
        Min
    max_001 : InputFloat
        Max
    min_002 : InputInteger
        Min
    max_002 : InputInteger
        Max
    probability : InputFloat
        Probability
    id : InputInteger
        ID
    seed : InputInteger
        Seed

    Outputs
    -------
    value : VectorSocket
        Value
    value_001 : FloatSocket
        Value
    value_002 : IntegerSocket
        Value
    value_003 : BooleanSocket
        Value
    """

    _bl_idname = "FunctionNodeRandomValue"
    node: bpy.types.FunctionNodeRandomValue

    class Inputs(SocketAccessor):
        min: Socket
        max: Socket
        min_001: Socket
        max_001: Socket
        min_002: Socket
        max_002: Socket
        probability: Socket
        id: Socket
        seed: Socket

    class Outputs(SocketAccessor):
        value: VectorSocket
        value_001: FloatSocket
        value_002: IntegerSocket
        value_003: BooleanSocket

    @property
    def i(self) -> "RandomValue.Inputs":
        return RandomValue.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "RandomValue.Outputs":
        return RandomValue.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        min: InputVector = None,
        max: InputVector = None,
        min_001: InputFloat = 0.0,
        max_001: InputFloat = 1.0,
        min_002: InputInteger = 0,
        max_002: InputInteger = 100,
        probability: InputFloat = 0.5,
        id: InputInteger = 0,
        seed: InputInteger = 0,
        *,
        data_type: Literal["FLOAT", "INT", "BOOLEAN", "FLOAT_VECTOR"] = "FLOAT",
    ):
        super().__init__()
        key_args = {
            "Min": min,
            "Max": max,
            "Min_001": min_001,
            "Max_001": max_001,
            "Min_002": min_002,
            "Max_002": max_002,
            "Probability": probability,
            "ID": id,
            "Seed": seed,
        }
        self.data_type = data_type
        self._establish_links(**key_args)

    @classmethod
    def float(
        cls,
        min: InputFloat = 0.0,
        max: InputFloat = 1.0,
        id: InputInteger = 0,
        seed: InputInteger = 0,
    ) -> "RandomValue":
        """Create Random Value with operation 'Float'. Floating-point value"""
        return cls(data_type="FLOAT", min_001=min, max_001=max, id=id, seed=seed)

    @classmethod
    def integer(
        cls,
        min: InputInteger = 0,
        max: InputInteger = 100,
        id: InputInteger = 0,
        seed: InputInteger = 0,
    ) -> "RandomValue":
        """Create Random Value with operation 'Integer'. 32-bit integer"""
        return cls(data_type="INT", min_002=min, max_002=max, id=id, seed=seed)

    @classmethod
    def boolean(
        cls, probability: InputFloat = 0.5, id: InputInteger = 0, seed: InputInteger = 0
    ) -> "RandomValue":
        """Create Random Value with operation 'Boolean'. True or false"""
        return cls(data_type="BOOLEAN", probability=probability, id=id, seed=seed)

    @classmethod
    def vector(
        cls,
        min: InputVector = None,
        max: InputVector = None,
        id: InputInteger = 0,
        seed: InputInteger = 0,
    ) -> "RandomValue":
        """Create Random Value with operation 'Vector'. 3D vector with floating-point values"""
        return cls(data_type="FLOAT_VECTOR", min=min, max=max, id=id, seed=seed)

    @property
    def data_type(self) -> Literal["FLOAT", "INT", "BOOLEAN", "FLOAT_VECTOR"]:
        return self.node.data_type

    @data_type.setter
    def data_type(self, value: Literal["FLOAT", "INT", "BOOLEAN", "FLOAT_VECTOR"]):
        self.node.data_type = value


class ReplaceString(NodeBuilder):
    """
    Replace a given string segment with another

    Parameters
    ----------
    string : InputString
        String
    find : InputString
        Find
    replace : InputString
        Replace

    Outputs
    -------
    string : StringSocket
        String
    """

    _bl_idname = "FunctionNodeReplaceString"
    node: bpy.types.FunctionNodeReplaceString

    class Inputs(SocketAccessor):
        string: Socket
        find: Socket
        replace: Socket

    class Outputs(SocketAccessor):
        string: StringSocket

    @property
    def i(self) -> "ReplaceString.Inputs":
        return ReplaceString.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "ReplaceString.Outputs":
        return ReplaceString.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        string: InputString = "",
        find: InputString = "",
        replace: InputString = "",
    ):
        super().__init__()
        key_args = {"String": string, "Find": find, "Replace": replace}

        self._establish_links(**key_args)


class RotateEuler(NodeBuilder):
    """
    Apply a secondary Euler rotation to a given Euler rotation

    Parameters
    ----------
    rotation : InputVector
        Rotation
    rotate_by : InputVector
        Rotate By
    axis : InputVector
        Axis
    angle : InputFloat
        Angle

    Outputs
    -------
    rotation : VectorSocket
        Rotation
    """

    _bl_idname = "FunctionNodeRotateEuler"
    node: bpy.types.FunctionNodeRotateEuler

    class Inputs(SocketAccessor):
        rotation: Socket
        rotate_by: Socket
        axis: Socket
        angle: Socket

    class Outputs(SocketAccessor):
        rotation: VectorSocket

    @property
    def i(self) -> "RotateEuler.Inputs":
        return RotateEuler.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "RotateEuler.Outputs":
        return RotateEuler.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        rotation: InputVector = None,
        rotate_by: InputVector = None,
        axis: InputVector = None,
        angle: InputFloat = 0.0,
        *,
        rotation_type: Literal["AXIS_ANGLE", "EULER"] = "EULER",
        space: Literal["OBJECT", "LOCAL"] = "OBJECT",
    ):
        super().__init__()
        key_args = {
            "Rotation": rotation,
            "Rotate By": rotate_by,
            "Axis": axis,
            "Angle": angle,
        }
        self.rotation_type = rotation_type
        self.space = space
        self._establish_links(**key_args)

    @classmethod
    def axis_angle(
        cls,
        rotation: InputVector = None,
        axis: InputVector = None,
        angle: InputFloat = 0.0,
    ) -> "RotateEuler":
        """Create Rotate Euler with operation 'Axis Angle'. Rotate around an axis by an angle"""
        return cls(
            rotation_type="AXIS_ANGLE", rotation=rotation, axis=axis, angle=angle
        )

    @classmethod
    def euler(
        cls, rotation: InputVector = None, rotate_by: InputVector = None
    ) -> "RotateEuler":
        """Create Rotate Euler with operation 'Euler'. Rotate around the X, Y, and Z axes"""
        return cls(rotation_type="EULER", rotation=rotation, rotate_by=rotate_by)

    @property
    def rotation_type(self) -> Literal["AXIS_ANGLE", "EULER"]:
        return self.node.rotation_type

    @rotation_type.setter
    def rotation_type(self, value: Literal["AXIS_ANGLE", "EULER"]):
        self.node.rotation_type = value

    @property
    def space(self) -> Literal["OBJECT", "LOCAL"]:
        return self.node.space

    @space.setter
    def space(self, value: Literal["OBJECT", "LOCAL"]):
        self.node.space = value


class RotateRotation(NodeBuilder):
    """
    Apply a secondary rotation to a given rotation value

    Parameters
    ----------
    rotation : InputRotation
        Rotation
    rotate_by : InputRotation
        Rotate By

    Outputs
    -------
    rotation : RotationSocket
        Rotation
    """

    _bl_idname = "FunctionNodeRotateRotation"
    node: bpy.types.FunctionNodeRotateRotation

    class Inputs(SocketAccessor):
        rotation: Socket
        rotate_by: Socket

    class Outputs(SocketAccessor):
        rotation: RotationSocket

    @property
    def i(self) -> "RotateRotation.Inputs":
        return RotateRotation.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "RotateRotation.Outputs":
        return RotateRotation.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        rotation: InputRotation = None,
        rotate_by: InputRotation = None,
        *,
        rotation_space: Literal["GLOBAL", "LOCAL"] = "GLOBAL",
    ):
        super().__init__()
        key_args = {"Rotation": rotation, "Rotate By": rotate_by}
        self.rotation_space = rotation_space
        self._establish_links(**key_args)

    @property
    def rotation_space(self) -> Literal["GLOBAL", "LOCAL"]:
        return self.node.rotation_space

    @rotation_space.setter
    def rotation_space(self, value: Literal["GLOBAL", "LOCAL"]):
        self.node.rotation_space = value


class RotateVector(NodeBuilder):
    """
    Apply a rotation to a given vector

    Parameters
    ----------
    vector : InputVector
        Vector
    rotation : InputRotation
        Rotation

    Outputs
    -------
    vector : VectorSocket
        Vector
    """

    _bl_idname = "FunctionNodeRotateVector"
    node: bpy.types.FunctionNodeRotateVector

    class Inputs(SocketAccessor):
        vector: Socket
        rotation: Socket

    class Outputs(SocketAccessor):
        vector: VectorSocket

    @property
    def i(self) -> "RotateVector.Inputs":
        return RotateVector.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "RotateVector.Outputs":
        return RotateVector.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        vector: InputVector = None,
        rotation: InputRotation = None,
    ):
        super().__init__()
        key_args = {"Vector": vector, "Rotation": rotation}

        self._establish_links(**key_args)


class RotationToAxisAngle(NodeBuilder):
    """
    Convert a rotation to axis angle components

    Parameters
    ----------
    rotation : InputRotation
        Rotation

    Outputs
    -------
    axis : VectorSocket
        Axis
    angle : FloatSocket
        Angle
    """

    _bl_idname = "FunctionNodeRotationToAxisAngle"
    node: bpy.types.FunctionNodeRotationToAxisAngle

    class Inputs(SocketAccessor):
        rotation: Socket

    class Outputs(SocketAccessor):
        axis: VectorSocket
        angle: FloatSocket

    @property
    def i(self) -> "RotationToAxisAngle.Inputs":
        return RotationToAxisAngle.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "RotationToAxisAngle.Outputs":
        return RotationToAxisAngle.Outputs(self.node.outputs, "output")

    def __init__(self, rotation: InputRotation = None):
        super().__init__()
        key_args = {"Rotation": rotation}

        self._establish_links(**key_args)


class RotationToEuler(NodeBuilder):
    """
    Convert a standard rotation value to an Euler rotation

    Parameters
    ----------
    rotation : InputRotation
        Rotation

    Outputs
    -------
    euler : VectorSocket
        Euler
    """

    _bl_idname = "FunctionNodeRotationToEuler"
    node: bpy.types.FunctionNodeRotationToEuler

    class Inputs(SocketAccessor):
        rotation: Socket

    class Outputs(SocketAccessor):
        euler: VectorSocket

    @property
    def i(self) -> "RotationToEuler.Inputs":
        return RotationToEuler.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "RotationToEuler.Outputs":
        return RotationToEuler.Outputs(self.node.outputs, "output")

    def __init__(self, rotation: InputRotation = None):
        super().__init__()
        key_args = {"Rotation": rotation}

        self._establish_links(**key_args)


class RotationToQuaternion(NodeBuilder):
    """
    Retrieve the quaternion components representing a rotation

    Parameters
    ----------
    rotation : InputRotation
        Rotation

    Outputs
    -------
    w : FloatSocket
        W
    x : FloatSocket
        X
    y : FloatSocket
        Y
    z : FloatSocket
        Z
    """

    _bl_idname = "FunctionNodeRotationToQuaternion"
    node: bpy.types.FunctionNodeRotationToQuaternion

    class Inputs(SocketAccessor):
        rotation: Socket

    class Outputs(SocketAccessor):
        w: FloatSocket
        x: FloatSocket
        y: FloatSocket
        z: FloatSocket

    @property
    def i(self) -> "RotationToQuaternion.Inputs":
        return RotationToQuaternion.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "RotationToQuaternion.Outputs":
        return RotationToQuaternion.Outputs(self.node.outputs, "output")

    def __init__(self, rotation: InputRotation = None):
        super().__init__()
        key_args = {"Rotation": rotation}

        self._establish_links(**key_args)


class SeparateBundle(NodeBuilder):
    """
    Split a bundle into multiple sockets.

    Parameters
    ----------
    bundle : InputBundle
        Bundle
    """

    _bl_idname = "NodeSeparateBundle"
    node: bpy.types.Node

    class Inputs(SocketAccessor):
        bundle: Socket

    class Outputs(SocketAccessor):
        pass

    @property
    def i(self) -> "SeparateBundle.Inputs":
        return SeparateBundle.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "SeparateBundle.Outputs":
        return SeparateBundle.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        bundle: InputBundle = None,
        *,
        define_signature: bool = False,
    ):
        super().__init__()
        key_args = {"Bundle": bundle}
        self.define_signature = define_signature
        self._establish_links(**key_args)

    @property
    def define_signature(self) -> bool:
        return self.node.define_signature

    @define_signature.setter
    def define_signature(self, value: bool):
        self.node.define_signature = value


class SeparateColor(NodeBuilder):
    """
    Split a color into separate channels, based on a particular color model

    Parameters
    ----------
    color : InputColor
        Color

    Outputs
    -------
    red : FloatSocket
        Red
    green : FloatSocket
        Green
    blue : FloatSocket
        Blue
    alpha : FloatSocket
        Alpha
    """

    _bl_idname = "FunctionNodeSeparateColor"
    node: bpy.types.FunctionNodeSeparateColor

    class Inputs(SocketAccessor):
        color: Socket

    class Outputs(SocketAccessor):
        red: FloatSocket
        green: FloatSocket
        blue: FloatSocket
        alpha: FloatSocket

    @property
    def i(self) -> "SeparateColor.Inputs":
        return SeparateColor.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "SeparateColor.Outputs":
        return SeparateColor.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        color: InputColor = None,
        *,
        mode: Literal["RGB", "HSV", "HSL"] = "RGB",
    ):
        super().__init__()
        key_args = {"Color": color}
        self.mode = mode
        self._establish_links(**key_args)

    @classmethod
    def rgb(cls, color: InputColor = None) -> "SeparateColor":
        """Create Separate Color with operation 'RGB'. Use RGB (Red, Green, Blue) color processing"""
        return cls(mode="RGB", color=color)

    @classmethod
    def hsv(cls, color: InputColor = None) -> "SeparateColor":
        """Create Separate Color with operation 'HSV'. Use HSV (Hue, Saturation, Value) color processing"""
        return cls(mode="HSV", color=color)

    @classmethod
    def hsl(cls, color: InputColor = None) -> "SeparateColor":
        """Create Separate Color with operation 'HSL'. Use HSL (Hue, Saturation, Lightness) color processing"""
        return cls(mode="HSL", color=color)

    @property
    def mode(self) -> Literal["RGB", "HSV", "HSL"]:
        return self.node.mode

    @mode.setter
    def mode(self, value: Literal["RGB", "HSV", "HSL"]):
        self.node.mode = value


class SeparateMatrix(NodeBuilder):
    """
    Split a 4x4 matrix into its individual values

    Parameters
    ----------
    matrix : InputMatrix
        Matrix

    Outputs
    -------
    column_1_row_1 : FloatSocket
        Column 1 Row 1
    column_1_row_2 : FloatSocket
        Column 1 Row 2
    column_1_row_3 : FloatSocket
        Column 1 Row 3
    column_1_row_4 : FloatSocket
        Column 1 Row 4
    column_2_row_1 : FloatSocket
        Column 2 Row 1
    column_2_row_2 : FloatSocket
        Column 2 Row 2
    column_2_row_3 : FloatSocket
        Column 2 Row 3
    column_2_row_4 : FloatSocket
        Column 2 Row 4
    column_3_row_1 : FloatSocket
        Column 3 Row 1
    column_3_row_2 : FloatSocket
        Column 3 Row 2
    column_3_row_3 : FloatSocket
        Column 3 Row 3
    column_3_row_4 : FloatSocket
        Column 3 Row 4
    column_4_row_1 : FloatSocket
        Column 4 Row 1
    column_4_row_2 : FloatSocket
        Column 4 Row 2
    column_4_row_3 : FloatSocket
        Column 4 Row 3
    column_4_row_4 : FloatSocket
        Column 4 Row 4
    """

    _bl_idname = "FunctionNodeSeparateMatrix"
    node: bpy.types.FunctionNodeSeparateMatrix

    class Inputs(SocketAccessor):
        matrix: Socket

    class Outputs(SocketAccessor):
        column_1_row_1: FloatSocket
        column_1_row_2: FloatSocket
        column_1_row_3: FloatSocket
        column_1_row_4: FloatSocket
        column_2_row_1: FloatSocket
        column_2_row_2: FloatSocket
        column_2_row_3: FloatSocket
        column_2_row_4: FloatSocket
        column_3_row_1: FloatSocket
        column_3_row_2: FloatSocket
        column_3_row_3: FloatSocket
        column_3_row_4: FloatSocket
        column_4_row_1: FloatSocket
        column_4_row_2: FloatSocket
        column_4_row_3: FloatSocket
        column_4_row_4: FloatSocket

    @property
    def i(self) -> "SeparateMatrix.Inputs":
        return SeparateMatrix.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "SeparateMatrix.Outputs":
        return SeparateMatrix.Outputs(self.node.outputs, "output")

    def __init__(self, matrix: InputMatrix = None):
        super().__init__()
        key_args = {"Matrix": matrix}

        self._establish_links(**key_args)


class SeparateTransform(NodeBuilder):
    """
    Split a transformation matrix into a translation vector, a rotation, and a scale vector

    Parameters
    ----------
    transform : InputMatrix
        Transform

    Outputs
    -------
    translation : VectorSocket
        Translation
    rotation : RotationSocket
        Rotation
    scale : VectorSocket
        Scale
    """

    _bl_idname = "FunctionNodeSeparateTransform"
    node: bpy.types.FunctionNodeSeparateTransform

    class Inputs(SocketAccessor):
        transform: Socket

    class Outputs(SocketAccessor):
        translation: VectorSocket
        rotation: RotationSocket
        scale: VectorSocket

    @property
    def i(self) -> "SeparateTransform.Inputs":
        return SeparateTransform.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "SeparateTransform.Outputs":
        return SeparateTransform.Outputs(self.node.outputs, "output")

    def __init__(self, transform: InputMatrix = None):
        super().__init__()
        key_args = {"Transform": transform}

        self._establish_links(**key_args)


class SeparateXYZ(NodeBuilder):
    """
    Split a vector into its X, Y, and Z components

    Parameters
    ----------
    vector : InputVector
        Vector

    Outputs
    -------
    x : FloatSocket
        X
    y : FloatSocket
        Y
    z : FloatSocket
        Z
    """

    _bl_idname = "ShaderNodeSeparateXYZ"
    node: bpy.types.ShaderNodeSeparateXYZ

    class Inputs(SocketAccessor):
        vector: Socket

    class Outputs(SocketAccessor):
        x: FloatSocket
        y: FloatSocket
        z: FloatSocket

    @property
    def i(self) -> "SeparateXYZ.Inputs":
        return SeparateXYZ.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "SeparateXYZ.Outputs":
        return SeparateXYZ.Outputs(self.node.outputs, "output")

    def __init__(self, vector: InputVector = None):
        super().__init__()
        key_args = {"Vector": vector}

        self._establish_links(**key_args)


class SliceString(NodeBuilder):
    """
    Extract a string segment from a larger string

    Parameters
    ----------
    string : InputString
        String
    position : InputInteger
        Position
    length : InputInteger
        Length

    Outputs
    -------
    string : StringSocket
        String
    """

    _bl_idname = "FunctionNodeSliceString"
    node: bpy.types.FunctionNodeSliceString

    class Inputs(SocketAccessor):
        string: Socket
        position: Socket
        length: Socket

    class Outputs(SocketAccessor):
        string: StringSocket

    @property
    def i(self) -> "SliceString.Inputs":
        return SliceString.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "SliceString.Outputs":
        return SliceString.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        string: InputString = "",
        position: InputInteger = 0,
        length: InputInteger = 10,
    ):
        super().__init__()
        key_args = {"String": string, "Position": position, "Length": length}

        self._establish_links(**key_args)


class StringLength(NodeBuilder):
    """
    Output the number of characters in the given string

    Parameters
    ----------
    string : InputString
        String

    Outputs
    -------
    length : IntegerSocket
        Length
    """

    _bl_idname = "FunctionNodeStringLength"
    node: bpy.types.FunctionNodeStringLength

    class Inputs(SocketAccessor):
        string: Socket

    class Outputs(SocketAccessor):
        length: IntegerSocket

    @property
    def i(self) -> "StringLength.Inputs":
        return StringLength.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "StringLength.Outputs":
        return StringLength.Outputs(self.node.outputs, "output")

    def __init__(self, string: InputString = ""):
        super().__init__()
        key_args = {"String": string}

        self._establish_links(**key_args)


class StringToValue(NodeBuilder):
    """
    Derive a numeric value from a given string representation

    Parameters
    ----------
    string : InputString
        String

    Outputs
    -------
    value : FloatSocket
        Value
    length : IntegerSocket
        Length
    """

    _bl_idname = "FunctionNodeStringToValue"
    node: bpy.types.FunctionNodeStringToValue

    class Inputs(SocketAccessor):
        string: Socket

    class Outputs(SocketAccessor):
        value: FloatSocket
        length: IntegerSocket

    @property
    def i(self) -> "StringToValue.Inputs":
        return StringToValue.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "StringToValue.Outputs":
        return StringToValue.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        string: InputString = "",
        *,
        data_type: Literal["FLOAT", "INT"] = "FLOAT",
    ):
        super().__init__()
        key_args = {"String": string}
        self.data_type = data_type
        self._establish_links(**key_args)

    @classmethod
    def float(cls, string: InputString = "") -> "StringToValue":
        """Create String to Value with operation 'Float'. Floating-point value"""
        return cls(data_type="FLOAT", string=string)

    @classmethod
    def integer(cls, string: InputString = "") -> "StringToValue":
        """Create String to Value with operation 'Integer'. 32-bit integer"""
        return cls(data_type="INT", string=string)

    @property
    def data_type(self) -> Literal["FLOAT", "INT"]:
        return self.node.data_type

    @data_type.setter
    def data_type(self, value: Literal["FLOAT", "INT"]):
        self.node.data_type = value


class Switch(NodeBuilder):
    """
    Switch between two inputs

    Parameters
    ----------
    switch : InputBoolean
        Switch
    false : InputGeometry
        False
    true : InputGeometry
        True

    Outputs
    -------
    output : GeometrySocket
        Output
    """

    _bl_idname = "GeometryNodeSwitch"
    node: bpy.types.GeometryNodeSwitch

    class Inputs(SocketAccessor):
        switch: Socket
        false: Socket
        true: Socket

    class Outputs(SocketAccessor):
        output: GeometrySocket

    @property
    def i(self) -> "Switch.Inputs":
        return Switch.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "Switch.Outputs":
        return Switch.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        switch: InputBoolean = False,
        false: InputGeometry = None,
        true: InputGeometry = None,
        *,
        input_type: Literal[
            "FLOAT",
            "INT",
            "BOOLEAN",
            "VECTOR",
            "RGBA",
            "ROTATION",
            "MATRIX",
            "STRING",
            "MENU",
            "OBJECT",
            "IMAGE",
            "GEOMETRY",
            "COLLECTION",
            "MATERIAL",
            "BUNDLE",
            "CLOSURE",
        ] = "GEOMETRY",
    ):
        super().__init__()
        key_args = {"Switch": switch, "False": false, "True": true}
        self.input_type = input_type
        self._establish_links(**key_args)

    @classmethod
    def float(
        cls,
        switch: InputBoolean = False,
        false: InputFloat = 0.0,
        true: InputFloat = 0.0,
    ) -> "Switch":
        """Create Switch with operation 'Float'."""
        return cls(input_type="FLOAT", switch=switch, false=false, true=true)

    @classmethod
    def integer(
        cls,
        switch: InputBoolean = False,
        false: InputInteger = 0,
        true: InputInteger = 0,
    ) -> "Switch":
        """Create Switch with operation 'Integer'."""
        return cls(input_type="INT", switch=switch, false=false, true=true)

    @classmethod
    def boolean(
        cls,
        switch: InputBoolean = False,
        false: InputBoolean = False,
        true: InputBoolean = False,
    ) -> "Switch":
        """Create Switch with operation 'Boolean'."""
        return cls(input_type="BOOLEAN", switch=switch, false=false, true=true)

    @classmethod
    def vector(
        cls,
        switch: InputBoolean = False,
        false: InputVector = None,
        true: InputVector = None,
    ) -> "Switch":
        """Create Switch with operation 'Vector'."""
        return cls(input_type="VECTOR", switch=switch, false=false, true=true)

    @classmethod
    def color(
        cls,
        switch: InputBoolean = False,
        false: InputColor = None,
        true: InputColor = None,
    ) -> "Switch":
        """Create Switch with operation 'Color'."""
        return cls(input_type="RGBA", switch=switch, false=false, true=true)

    @classmethod
    def rotation(
        cls,
        switch: InputBoolean = False,
        false: InputRotation = None,
        true: InputRotation = None,
    ) -> "Switch":
        """Create Switch with operation 'Rotation'."""
        return cls(input_type="ROTATION", switch=switch, false=false, true=true)

    @classmethod
    def matrix(
        cls,
        switch: InputBoolean = False,
        false: InputMatrix = None,
        true: InputMatrix = None,
    ) -> "Switch":
        """Create Switch with operation 'Matrix'."""
        return cls(input_type="MATRIX", switch=switch, false=false, true=true)

    @classmethod
    def string(
        cls,
        switch: InputBoolean = False,
        false: InputString = "",
        true: InputString = "",
    ) -> "Switch":
        """Create Switch with operation 'String'."""
        return cls(input_type="STRING", switch=switch, false=false, true=true)

    @classmethod
    def menu(
        cls,
        switch: InputBoolean = False,
        false: InputMenu = None,
        true: InputMenu = None,
    ) -> "Switch":
        """Create Switch with operation 'Menu'."""
        return cls(input_type="MENU", switch=switch, false=false, true=true)

    @classmethod
    def object(
        cls,
        switch: InputBoolean = False,
        false: InputObject = None,
        true: InputObject = None,
    ) -> "Switch":
        """Create Switch with operation 'Object'."""
        return cls(input_type="OBJECT", switch=switch, false=false, true=true)

    @classmethod
    def image(
        cls,
        switch: InputBoolean = False,
        false: InputImage = None,
        true: InputImage = None,
    ) -> "Switch":
        """Create Switch with operation 'Image'."""
        return cls(input_type="IMAGE", switch=switch, false=false, true=true)

    @classmethod
    def geometry(
        cls,
        switch: InputBoolean = False,
        false: InputGeometry = None,
        true: InputGeometry = None,
    ) -> "Switch":
        """Create Switch with operation 'Geometry'."""
        return cls(input_type="GEOMETRY", switch=switch, false=false, true=true)

    @classmethod
    def collection(
        cls,
        switch: InputBoolean = False,
        false: InputCollection = None,
        true: InputCollection = None,
    ) -> "Switch":
        """Create Switch with operation 'Collection'."""
        return cls(input_type="COLLECTION", switch=switch, false=false, true=true)

    @classmethod
    def material(
        cls,
        switch: InputBoolean = False,
        false: InputMaterial = None,
        true: InputMaterial = None,
    ) -> "Switch":
        """Create Switch with operation 'Material'."""
        return cls(input_type="MATERIAL", switch=switch, false=false, true=true)

    @classmethod
    def bundle(
        cls,
        switch: InputBoolean = False,
        false: InputBundle = None,
        true: InputBundle = None,
    ) -> "Switch":
        """Create Switch with operation 'Bundle'."""
        return cls(input_type="BUNDLE", switch=switch, false=false, true=true)

    @classmethod
    def closure(
        cls,
        switch: InputBoolean = False,
        false: InputClosure = None,
        true: InputClosure = None,
    ) -> "Switch":
        """Create Switch with operation 'Closure'."""
        return cls(input_type="CLOSURE", switch=switch, false=false, true=true)

    @property
    def input_type(
        self,
    ) -> Literal[
        "FLOAT",
        "INT",
        "BOOLEAN",
        "VECTOR",
        "RGBA",
        "ROTATION",
        "MATRIX",
        "STRING",
        "MENU",
        "OBJECT",
        "IMAGE",
        "GEOMETRY",
        "COLLECTION",
        "MATERIAL",
        "BUNDLE",
        "CLOSURE",
    ]:
        return self.node.input_type

    @input_type.setter
    def input_type(
        self,
        value: Literal[
            "FLOAT",
            "INT",
            "BOOLEAN",
            "VECTOR",
            "RGBA",
            "ROTATION",
            "MATRIX",
            "STRING",
            "MENU",
            "OBJECT",
            "IMAGE",
            "GEOMETRY",
            "COLLECTION",
            "MATERIAL",
            "BUNDLE",
            "CLOSURE",
        ],
    ):
        self.node.input_type = value


class TransformDirection(NodeBuilder):
    """
    Apply a transformation matrix (excluding translation) to the given vector

    Parameters
    ----------
    direction : InputVector
        Direction
    transform : InputMatrix
        Transform

    Outputs
    -------
    direction : VectorSocket
        Direction
    """

    _bl_idname = "FunctionNodeTransformDirection"
    node: bpy.types.FunctionNodeTransformDirection

    class Inputs(SocketAccessor):
        direction: Socket
        transform: Socket

    class Outputs(SocketAccessor):
        direction: VectorSocket

    @property
    def i(self) -> "TransformDirection.Inputs":
        return TransformDirection.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "TransformDirection.Outputs":
        return TransformDirection.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        direction: InputVector = None,
        transform: InputMatrix = None,
    ):
        super().__init__()
        key_args = {"Direction": direction, "Transform": transform}

        self._establish_links(**key_args)


class TransformPoint(NodeBuilder):
    """
    Apply a transformation matrix to the given vector

    Parameters
    ----------
    vector : InputVector
        Vector
    transform : InputMatrix
        Transform

    Outputs
    -------
    vector : VectorSocket
        Vector
    """

    _bl_idname = "FunctionNodeTransformPoint"
    node: bpy.types.FunctionNodeTransformPoint

    class Inputs(SocketAccessor):
        vector: Socket
        transform: Socket

    class Outputs(SocketAccessor):
        vector: VectorSocket

    @property
    def i(self) -> "TransformPoint.Inputs":
        return TransformPoint.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "TransformPoint.Outputs":
        return TransformPoint.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        vector: InputVector = None,
        transform: InputMatrix = None,
    ):
        super().__init__()
        key_args = {"Vector": vector, "Transform": transform}

        self._establish_links(**key_args)


class TransposeMatrix(NodeBuilder):
    """
    Flip a matrix over its diagonal, turning columns into rows and vice-versa

    Parameters
    ----------
    matrix : InputMatrix
        Matrix

    Outputs
    -------
    matrix : MatrixSocket
        Matrix
    """

    _bl_idname = "FunctionNodeTransposeMatrix"
    node: bpy.types.FunctionNodeTransposeMatrix

    class Inputs(SocketAccessor):
        matrix: Socket

    class Outputs(SocketAccessor):
        matrix: MatrixSocket

    @property
    def i(self) -> "TransposeMatrix.Inputs":
        return TransposeMatrix.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "TransposeMatrix.Outputs":
        return TransposeMatrix.Outputs(self.node.outputs, "output")

    def __init__(self, matrix: InputMatrix = None):
        super().__init__()
        key_args = {"Matrix": matrix}

        self._establish_links(**key_args)


class UVUnwrap(NodeBuilder):
    """
    Generate a UV map based on seam edges

    Parameters
    ----------
    selection : InputBoolean
        Selection
    seam : InputBoolean
        Seam
    margin : InputFloat
        Margin
    fill_holes : InputBoolean
        Fill Holes
    method : InputMenu | Literal['Angle Based', 'Conformal']
        Method

    Outputs
    -------
    uv : VectorSocket
        UV
    """

    _bl_idname = "GeometryNodeUVUnwrap"
    node: bpy.types.GeometryNodeUVUnwrap

    class Inputs(SocketAccessor):
        selection: Socket
        seam: Socket
        margin: Socket
        fill_holes: Socket
        method: Socket

    class Outputs(SocketAccessor):
        uv: VectorSocket

    @property
    def i(self) -> "UVUnwrap.Inputs":
        return UVUnwrap.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "UVUnwrap.Outputs":
        return UVUnwrap.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        selection: InputBoolean = True,
        seam: InputBoolean = False,
        margin: InputFloat = 0.001,
        fill_holes: InputBoolean = True,
        method: InputMenu | Literal["Angle Based", "Conformal"] = "Angle Based",
    ):
        super().__init__()
        key_args = {
            "Selection": selection,
            "Seam": seam,
            "Margin": margin,
            "Fill Holes": fill_holes,
            "Method": method,
        }

        self._establish_links(**key_args)


class ValueToString(NodeBuilder):
    """
    Generate a string representation of the given input value

    Parameters
    ----------
    value : InputFloat
        Value
    decimals : InputInteger
        Decimals

    Outputs
    -------
    string : StringSocket
        String
    """

    _bl_idname = "FunctionNodeValueToString"
    node: bpy.types.FunctionNodeValueToString

    class Inputs(SocketAccessor):
        value: Socket
        decimals: Socket

    class Outputs(SocketAccessor):
        string: StringSocket

    @property
    def i(self) -> "ValueToString.Inputs":
        return ValueToString.Inputs(self.node.inputs, "input")

    @property
    def o(self) -> "ValueToString.Outputs":
        return ValueToString.Outputs(self.node.outputs, "output")

    def __init__(
        self,
        value: InputFloat = 0.0,
        decimals: InputInteger = 0,
        *,
        data_type: Literal["FLOAT", "INT"] = "FLOAT",
    ):
        super().__init__()
        key_args = {"Value": value, "Decimals": decimals}
        self.data_type = data_type
        self._establish_links(**key_args)

    @classmethod
    def float(
        cls, value: InputFloat = 0.0, decimals: InputInteger = 0
    ) -> "ValueToString":
        """Create Value to String with operation 'Float'. Floating-point value"""
        return cls(data_type="FLOAT", value=value, decimals=decimals)

    @classmethod
    def integer(cls, value: InputInteger = 0) -> "ValueToString":
        """Create Value to String with operation 'Integer'. 32-bit integer"""
        return cls(data_type="INT", value=value)

    @property
    def data_type(self) -> Literal["FLOAT", "INT"]:
        return self.node.data_type

    @data_type.setter
    def data_type(self, value: Literal["FLOAT", "INT"]):
        self.node.data_type = value
