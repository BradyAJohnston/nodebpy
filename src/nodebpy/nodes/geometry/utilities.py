# Auto-generated by generate.py — do not edit manually.

from typing import TYPE_CHECKING, Any

import bpy
from bpy.types import NodeSocket

from ...builder import BaseNode as NodeBuilder, SocketAccessor, ColorSocket
from ...builder._utils import SocketError

from ...types import (
    InputColor,
)


class Reroute(NodeBuilder):
    """
    A single-socket organization tool that supports one input and multiple outputs

    Parameters
    ----------
    input : InputColor
        Input

    Inputs
    ------
    i.input : ColorSocket
        Input

    Outputs
    -------
    o.output : ColorSocket
        Output
    """

    _bl_idname = "NodeReroute"
    node: bpy.types.Node

    class _Inputs(SocketAccessor):
        input: ColorSocket
        """Input"""

    class _Outputs(SocketAccessor):
        output: ColorSocket
        """Output"""

    if TYPE_CHECKING:

        @property
        def i(self) -> _Inputs: ...
        @property
        def o(self) -> _Outputs: ...

    def __init__(
        self,
        input: InputColor = None,
        *,
        socket_idname: str = "",
    ):
        super().__init__()
        key_args = {"Input": input}
        self.socket_idname = socket_idname
        self._establish_links(**key_args)

    @property
    def socket_idname(self) -> str:
        return self.node.socket_idname

    @socket_idname.setter
    def socket_idname(self, value: str):
        self.node.socket_idname = value

    def _find_best_socket_pair(self, source: Any, target: Any) -> tuple[NodeSocket, NodeSocket]:
        if hasattr(source, "outputs"):
            outputs = source.outputs._available
        elif isinstance(source, NodeSocket):
            outputs = [source]
        else:
            raise SocketError(f"Cannot get outputs from {type(source)}")
        if outputs:
            return self.node.inputs[0], outputs[0]
        raise SocketError(f"No available outputs from {source}")
